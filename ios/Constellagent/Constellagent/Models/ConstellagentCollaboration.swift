// FILE: ConstellagentCollaboration.swift
// Purpose: Shared collaboration-mode and multi-agent models used by composer sends and timeline rendering.
// Layer: Model
// Exports: ConstellagentCollaborationModeKind, ConstellagentPlanState, ConstellagentStructuredUserInputRequest,
//   ConstellagentSubagentAction
// Depends on: Foundation, JSONValue

import Foundation

enum ConstellagentCollaborationModeKind: String, Codable, Hashable, Sendable {
    case `default`
    case plan
}

enum ConstellagentRuntimeTransportMode: String, Codable, Hashable, Sendable {
    case unknown
    case websocket
    case spawn
}

enum ConstellagentPlanSessionSource: String, Codable, Hashable, Sendable {
    case requested
...
struct ConstellagentProposedPlan: Codable, Hashable, Sendable {
    let body: String
    let summary: String?
...
enum ConstellagentPlanPresentation: String, Codable, Hashable, Sendable {
    case progress
...
enum ConstellagentPlanStepStatus: String, Codable, Hashable, Sendable {
    case pending
...
struct ConstellagentPlanStep: Identifiable, Codable, Hashable, Sendable {
    let id: String
    let step: String
    let status: ConstellagentPlanStepStatus

    init(id: String = UUID().uuidString, step: String, status: ConstellagentPlanStepStatus) {
        self.id = id
        self.step = step
        self.status = status
    }
...
struct ConstellagentPlanState: Codable, Hashable, Sendable {
    var explanation: String?
    var steps: [ConstellagentPlanStep]

    init(explanation: String? = nil, steps: [ConstellagentPlanStep] = []) {
        self.explanation = explanation
        self.steps = steps
    }
}

enum ConstellagentProposedPlanParser {
    ...
    static func parse(from rawText: String) -> ConstellagentProposedPlan? {
        ...
        return ConstellagentProposedPlan(body: body, summary: proposedPlanSummary(from: body))
    }
    ...
    static func parseAssistantFallback(from rawText: String) -> ConstellagentProposedPlan? {
    ...
    static func parsePlanItem(from rawText: String) -> ConstellagentProposedPlan? {
        let body = rawText.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !body.isEmpty else {
            return nil
        }
...
nonisolated struct ConstellagentStructuredUserInputOption: Identifiable, Codable, Hashable, Sendable {
    let id: String
    let label: String
...
nonisolated struct ConstellagentStructuredUserInputQuestion: Identifiable, Codable, Hashable, Sendable {
    let id: String
    let header: String
    let question: String
    let isOther: Bool
    ...
    let options: [ConstellagentStructuredUserInputOption]
    ...
    init(
        ...
        options: [ConstellagentStructuredUserInputOption]
    ) {
        self.id = id
        self.header = header
...
struct ConstellagentStructuredUserInputRequest: Codable, Hashable, Sendable {
    let requestID: JSONValue
    let questions: [ConstellagentStructuredUserInputQuestion]

    init(requestID: JSONValue, questions: [ConstellagentStructuredUserInputQuestion]) {
        self.requestID = requestID
        self.questions = questions
    }
}

struct ConstellagentSubagentRef: Codable, Hashable, Sendable {
    let threadId: String
    let agentId: String?
    let nickname: String?
    let role: String?
...
struct ConstellagentSubagentState: Codable, Hashable, Sendable {
    let threadId: String
    var status: String
    var message: String?
...
struct ConstellagentSubagentThreadPresentation: Identifiable, Hashable, Sendable {
    let threadId: String
    let agentId: String?
    let nickname: String?
    let role: String?
...
struct ConstellagentSubagentAction: Codable, Hashable, Sendable {
    var tool: String
    var status: String
    var prompt: String?
    var model: String?
    var receiverThreadIds: [String]
    var receiverAgents: [ConstellagentSubagentRef]
    var agentStates: [String: ConstellagentSubagentState]
    ...
    init(
        ...
        receiverAgents: [ConstellagentSubagentRef] = [],
        agentStates: [String: ConstellagentSubagentState] = [:]
    ) {
        self.tool = tool
        self.status = status
        self.prompt = prompt
    ...
    var agentRows: [ConstellagentSubagentThreadPresentation] {
