// FILE: ConstellagentThread.swift
// Purpose: Represents a Constellagent conversation thread returned by thread/list and related events,
//   including native subagent identity metadata used by the sidebar and parent-child navigation.
// Layer: Model
// Exports: ConstellagentThread
// Depends on: JSONValue

import Foundation

enum ConstellagentTimestampParser {
...
enum ConstellagentThreadSyncState: String, Codable, Hashable, Sendable {
    case live
    case archivedLocal
}

struct ConstellagentThread: Identifiable, Codable, Hashable, Sendable {
    let id: String
    var title: String?
    var name: String?
    var preview: String?
    ...
    var syncState: ConstellagentThreadSyncState
    ...
    init(
        ...
        syncState: ConstellagentThreadSyncState = .live
    ) {
        self.id = id
        self.title = title
        self.name = name
    ...
    init(from decoder: Decoder) throws {
        ...
        syncState = try container.decodeIfPresent(ConstellagentThreadSyncState.self, forKey: .syncState) ?? .live
    }
...
extension ConstellagentThread {
    ...
    // Distinguishes Constellagent-managed worktrees from the main repo in compact sidebar UIs.
    static func projectDisplayLabel(for normalizedProjectPath: String?) -> String {
        ...
        guard let worktreeToken = constellagentManagedWorktreeDisplayToken(for: normalizedProjectPath) else {
            return baseLabel
        }
    ...
    static func projectIconSystemName(for normalizedProjectPath: String?) -> String {
        ...
        return constellagentManagedWorktreeToken(for: normalizedProjectPath) == nil ? "folder" : "arrow.triangle.branch"
    }
    ...
    private static func decodeDateIfPresent(
        ...
        for key in keys {
            if let stringValue = try? container.decodeIfPresent(String.self, forKey: key) {
                if let parsedDate = ConstellagentTimestampParser.parseString(stringValue) {
                    return parsedDate
                }
            }

            if let doubleValue = try? container.decodeIfPresent(Double.self, forKey: key) {
                return ConstellagentTimestampParser.decodeUnixTimestamp(doubleValue)
            }

            if let intValue = try? container.decodeIfPresent(Int64.self, forKey: key) {
                return ConstellagentTimestampParser.decodeUnixTimestamp(Double(intValue))
            }
    ...
    private static func constellagentManagedWorktreeToken(for normalizedProjectPath: String) -> String? {
        ...
        guard let worktreesIndex = components.firstIndex(of: "worktrees"),
              worktreesIndex > 0,
              components[worktreesIndex - 1] == ".constellagent" else {
            return nil
        }
    ...
    private static func constellagentManagedWorktreeDisplayToken(for normalizedProjectPath: String) -> String? {
        guard let token = constellagentManagedWorktreeToken(for: normalizedProjectPath) else {
            return nil
        }
