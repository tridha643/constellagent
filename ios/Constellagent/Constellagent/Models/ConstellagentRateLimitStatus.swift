// FILE: ConstellagentRateLimitStatus.swift
// Purpose: Models ChatGPT rate-limit buckets shown in the in-app status sheet.
// Layer: Model
// Exports: ConstellagentRateLimitBucket, ConstellagentRateLimitWindow, ConstellagentRateLimitDisplayRow

import Foundation

struct ConstellagentRateLimitWindow: Equatable, Sendable {
    let usedPercent: Int
...
struct ConstellagentRateLimitDisplayRow: Identifiable, Equatable, Sendable {
    let id: String
    let label: String
    let window: ConstellagentRateLimitWindow
}

struct ConstellagentRateLimitBucket: Identifiable, Equatable, Sendable {
    let limitId: String
    let limitName: String?
    let primary: ConstellagentRateLimitWindow?
    let secondary: ConstellagentRateLimitWindow?

    var id: String { limitId }

    var primaryOrSecondary: ConstellagentRateLimitWindow? {
        primary ?? secondary
    }

    // Splits combined primary/secondary windows into the rows the status sheet should actually render.
    var displayRows: [ConstellagentRateLimitDisplayRow] {
        var rows: [ConstellagentRateLimitDisplayRow] = []

        if let primary {
            rows.append(
                ConstellagentRateLimitDisplayRow(
                    id: "\(limitId)-primary",
    ...
    static func visibleDisplayRows(from buckets: [ConstellagentRateLimitBucket]) -> [ConstellagentRateLimitDisplayRow] {
        let rows = buckets.flatMap(\.displayRows)
        var dedupedByLabel: [String: ConstellagentRateLimitDisplayRow] = [:]
    ...
    private static func label(for window: ConstellagentRateLimitWindow, fallback: String) -> String {
        durationLabel(minutes: window.windowDurationMins) ?? fallback
    }

    private static func preferredDisplayRow(
        _ current: ConstellagentRateLimitDisplayRow,
        _ candidate: ConstellagentRateLimitDisplayRow
    ) -> ConstellagentRateLimitDisplayRow {
