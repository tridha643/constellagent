...
struct QRScannerView: View {
    let onBack: (() -> Void)?
    let onScan: (ConstellagentPairingQRPayload) -> Void

    @State private var scannerError: String?
    @State private var bridgeUpdatePrompt: ConstellagentBridgeUpdatePrompt?
    @State private var didCopyBridgeUpdateCommand = false
    @State private var hasCameraPermission = false
    @State private var isCheckingPermission = true

    init(
        initialBridgeUpdatePrompt: ConstellagentBridgeUpdatePrompt? = nil,
        initialHasCameraPermission: Bool = false,
        initialIsCheckingPermission: Bool = true,
        onBack: (() -> Void)? = nil,
        onScan: @escaping (ConstellagentPairingQRPayload) -> Void
    ) {
        self.onBack = onBack
        self.onScan = onScan
    ...
    private func bridgeUpdateView(prompt: ConstellagentBridgeUpdatePrompt) -> some View {
        VStack(alignment: .leading, spacing: 24) {
            ...
            VStack(alignment: .leading, spacing: 14) {
                if let command = prompt.command, !command.isEmpty {
                    ...
                    bridgeUpdateStep(number: "1", title: "Update Constellagent", detail: command, showsCopyButton: true)
                    bridgeUpdateStep(number: "2", title: "Start it again", detail: "Run constellagent up")
                    ...
                    bridgeUpdateStep(number: "1", title: "Update Constellagent", detail: "Install the latest Constellagent build on this iPhone.")
    ...
    private var scannerOverlay: some View {
        VStack(spacing: 24) {
            ...
            Text("Scan the Constellagent QR code")
    ...
    private var cameraPermissionView: some View {
        VStack(spacing: 20) {
            ConstellagentIcon.image(systemName: "camera.fill")
                .font(.system(size: 48))
...
private extension ConstellagentBridgeUpdatePrompt {
    static let previewScannerMismatch = ConstellagentBridgeUpdatePrompt(
