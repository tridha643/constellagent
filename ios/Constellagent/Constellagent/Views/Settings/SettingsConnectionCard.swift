...
// Depends on: SwiftUI, ConstellagentService connection state, SettingsSupportCards
...
struct SettingsConnectionCard: View {
    ...
    var body: some View {
        SettingsCard(title: "Connection") {
            if let trustedPairPresentation = constellagent.trustedPairPresentation {
            ...
            if case .retrying(_, let message) = constellagent.connectionRecoveryState,
               !message.isEmpty {
                Text(message)
                    .font(AppFont.caption())
                    .foregroundStyle(.secondary)
            ...
            if let error = constellagent.lastErrorMessage, !error.isEmpty {
                Text(error)
                    .font(AppFont.caption())
                    .foregroundStyle(.red)
            }

            if constellagent.supportsKeepAwakeWhileBridgeRuns {
                Toggle("Keep device reachable", isOn: keepMacAwakeWhileBridgeRunsBinding)
                    .tint(settingsAccentColor)

                    Text(constellagent.keepMacAwakeWhileBridgeRuns
            ...
            } else if constellagent.hasTrustedMacReconnectCandidate {
                SettingsButton("Forget Pair", role: .destructive) {
                    HapticFeedback.shared.triggerImpactFeedback()
                    constellagent.forgetTrustedMac()
                }
            }
        }
    ...
    private var keepMacAwakeWhileBridgeRunsBinding: Binding<Bool> {
        Binding(
            get: { constellagent.keepMacAwakeWhileBridgeRuns },
            set: { nextValue in
                constellagent.setKeepMacAwakeWhileBridgeRunsPreference(nextValue)
                Task { @MainActor in
                    await constellagent.syncBridgeKeepMacAwakePreferenceIfNeeded(showFailureInUI: true)
                }
            }
        )
    }
    ...
    private var connectionPhaseShowsProgress: Bool {
        switch constellagent.connectionPhase {
    ...
    private func sidebarComputerNicknameBinding(for presentation: ConstellagentTrustedPairPresentation) -> Binding<String> {
        Binding(
