...
// Depends on: SwiftUI, ConstellagentService project folder RPC helpers
...
struct SidebarLocalFolderBrowserSheet: View {
    ...
    @State private var quickLocations: [ConstellagentProjectLocation] = []
    @State private var currentPath: String?
    @State private var parentPath: String?
    @State private var entries: [ConstellagentProjectDirectoryEntry] = []
    ...
    @State private var searchResults: [ConstellagentProjectDirectoryEntry] = []
            ...
            .toolbar {
                ...
                ToolbarItemGroup(placement: .primaryAction) {
                    Button(action: presentNewFolderPrompt) {
                        ConstellagentIcon.image(systemName: "folder.badge.plus")
                    }
    ...
    private func loadInitialDirectory() async {
        ...
        do {
            let locations = try await constellagent.fetchProjectQuickLocations()
    ...
    private func loadDirectory(_ path: String) async {
        ...
        do {
            let listing = try await constellagent.listProjectDirectory(path: path)
    ...
    private func updateFolderSearch() async {
        ...
        do {
            ...
            let results = try await constellagent.searchProjectDirectories(rootPath: currentPath, query: query)
    ...
    private func createFolderAndSelect() async {
        ...
        do {
            let createdPath = try await constellagent.createProjectDirectory(
...
private struct SidebarLocalFolderLocationsSection: View {
    let locations: [ConstellagentProjectLocation]
...
private struct SidebarLocalFolderEntriesSection: View {
    ...
    let searchResults: [ConstellagentProjectDirectoryEntry]
    ...
    private func folderButton(_ entry: ConstellagentProjectDirectoryEntry) -> some View {
        Button {
            onSelect(entry.path)
        } label: {
            SidebarLocalFolderRow(
...
private struct SidebarLocalFolderRow: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            ConstellagentIcon.image(systemName: iconSystemName)
