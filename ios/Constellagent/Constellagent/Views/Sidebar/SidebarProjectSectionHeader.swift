...
//             SidebarSectionExpansionChevron, SidebarThreadGroup, ConstellagentIcon,
//             ConstellagentWorktreeIcon, AppFont, HapticFeedback
...
struct SidebarProjectSectionHeader: View {
    ...
    private var leadingIcon: some View {
        if group.iconSystemName == "arrow.triangle.branch" {
            ConstellagentWorktreeIcon(pointSize: 16, weight: .medium)
                .foregroundStyle(.primary)
        } else {
            ConstellagentIcon.image(systemName: resolvedIconName)
    ...
    private var composeButton: some View {
        ...
        HapticButton(hapticStyle: .medium, action: onCreate) {
            ConstellagentIcon.image(systemName: "square.and.pencil", size: 20, weight: .medium)
        ...
        if let onArchive {
            children.append(
                UIAction(
                    ...
                    image: ConstellagentIcon.menuUIImage(systemName: "archivebox")
                ) { _ in
        ...
        if let onDelete {
            children.append(
                UIAction(
                    ...
                    image: ConstellagentIcon.menuUIImage(systemName: "trash"),
