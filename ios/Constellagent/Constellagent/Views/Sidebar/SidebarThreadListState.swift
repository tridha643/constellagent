...
// Depends on: Foundation, ConstellagentThread, SidebarThreadGroup
...
enum SidebarProjectThreadPreviewState {
    ...
    static func rootThreads(in groupThreads: [ConstellagentThread]) -> [ConstellagentThread] {
    ...
    static func selectedThreadRequiresExpansion(
        _ selectedThread: ConstellagentThread?,
        in group: SidebarThreadGroup,
        rootThreads: [ConstellagentThread]? = nil
    ) -> Bool {
    ...
    private static func shouldRevealAllRootThreads(
        for group: SidebarThreadGroup,
        rootThreads: [ConstellagentThread],
        selectedThread: ConstellagentThread?,
        isFiltering: Bool,
        manuallyExpandedGroupIDs: Set<String>
    ) -> Bool {
    ...
    private static func rootThreadID(containing thread: ConstellagentThread, in groupThreads: [ConstellagentThread]) -> String? {
...
struct SidebarSubagentHierarchy {
    let rootThreads: [ConstellagentThread]
    let childrenByParentID: [String: [ConstellagentThread]]

    init(groupThreads: [ConstellagentThread]) {
        let threadsByID = Dictionary(uniqueKeysWithValues: groupThreads.map { ($0.id, $0) })
        var childrenByParentID: [String: [ConstellagentThread]] = [:]
        var rootThreads: [ConstellagentThread] = []
...
enum SidebarProjectExpansionState {
    ...
    static func groupIDContainingSelectedThread(_ selectedThread: ConstellagentThread?, in groups: [SidebarThreadGroup]) -> String? {
