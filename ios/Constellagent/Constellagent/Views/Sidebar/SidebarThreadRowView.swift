...
struct SidebarThreadRowView: View {
    ...
    let isSelected: Bool
    let runBadgeState: ConstellagentThreadRunBadgeState?
    ...
    // Read here only so we can re-inject `constellagent` across the
    ...
    // fault on `@Environment(ConstellagentService.self)` inside the wrapped row.
    // The row body itself never touches any property on `constellagent`, so the
    ...
    // subscribes to `constellagent.subagentIdentityVersion`).
    ...
    var body: some View {
        ...
        // Re-inject `constellagent` so `SidebarSubagentNameLabel` (which lives
        // inside the row body and reads `@Environment(ConstellagentService.self)`)
    ...
    // MARK: - Parent row (no ConstellagentService dependency)
    ...
    // MARK: - Subagent row (ConstellagentService isolated in SubagentNameLabel)
    ...
    private var expansionToggleButton: some View {
        if childSubagentCount > 0, let onToggleSubagents {
            HapticButton(action: onToggleSubagents) {
                ConstellagentIcon.image(systemName: isSubagentExpanded ? "chevron.down" : "chevron.right")
...
// MARK: - Subagent name label (isolates ConstellagentService observation)

/// Owns the `@Environment(ConstellagentService.self)` so parent thread rows
...
private struct SidebarSubagentNameLabel: View {
    ...
    var body: some View {
        let _ = constellagent.subagentIdentityVersion
        let source = thread.preferredSubagentLabel
            ?? constellagent.resolvedSubagentDisplayLabel(threadId: thread.id, agentId: thread.agentId)
            ?? "Subagent"
        let parsed = SubagentLabelParser.parse(source)
        let nickname = parsed.nickname.isEmpty || ConstellagentThread.isGenericPlaceholderTitle(parsed.nickname) ? "Subagent" : parsed.nickname
...
private enum SidebarRowPreviewFixtures {
    ...
    static let allThreads: [ConstellagentThread] = [
        // ── Project 1: auth-middleware ──
        ConstellagentThread(id: "t1", title: "Refactor auth middleware", createdAt: now.addingTimeInterval(-3600), updatedAt: now.addingTimeInterval(-60), cwd: "/Users/dev/auth-middleware"),
        ConstellagentThread(id: "t1_a", title: "Gibbs [explorer]", createdAt: now.addingTimeInterval(-3000), updatedAt: now.addingTimeInterval(-120), cwd: "/Users/dev/auth-middleware", parentThreadId: "t1", agentNickname: "Gibbs", agentRole: "explorer"),
        ConstellagentThread(id: "t1_b", title: "Locke [coder]", createdAt: now.addingTimeInterval(-2400), updatedAt: now.addingTimeInterval(-90), cwd: "/Users/dev/auth-middleware", parentThreadId: "t1", agentNickname: "Locke", agentRole: "coder"),
        ConstellagentThread(id: "t1_c", title: "Reyes [reviewer]", createdAt: now.addingTimeInterval(-1800), updatedAt: now.addingTimeInterval(-300), cwd: "/Users/dev/auth-middleware", parentThreadId: "t1", agentNickname: "Reyes", agentRole: "reviewer"),
        ConstellagentThread(id: "t2", title: "Add rate limiting", createdAt: now.addingTimeInterval(-7200), updatedAt: now.addingTimeInterval(-600), cwd: "/Users/dev/auth-middleware"),

        // ── Project 2: payments ──
        ConstellagentThread(id: "t3", title: "Fix payment flow", createdAt: now.addingTimeInterval(-14400), updatedAt: now.addingTimeInterval(-1200), cwd: "/Users/dev/payments"),
        ConstellagentThread(id: "t3_a", title: "Ford [planner]", createdAt: now.addingTimeInterval(-13000), updatedAt: now.addingTimeInterval(-1500), cwd: "/Users/dev/payments", parentThreadId: "t3", agentNickname: "Ford", agentRole: "planner"),
        ConstellagentThread(id: "t4", title: "Stripe webhook retry logic", createdAt: now.addingTimeInterval(-86400), updatedAt: now.addingTimeInterval(-3600), cwd: "/Users/dev/payments"),
    ]
    ...
    static let runBadges: [String: ConstellagentThreadRunBadgeState] = [
        "t1": .running,
        "t1_a": .running,
        "t1_b": .ready,
        "t3": .ready,
    ...
    static func timingLabel(for thread: ConstellagentThread) -> String? {
