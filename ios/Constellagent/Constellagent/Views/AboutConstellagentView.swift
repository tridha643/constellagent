// FILE: AboutConstellagentView.swift
// Purpose: Full-screen guide explaining how Constellagent works, styled as a blog page.
// Layer: View
// Exports: AboutConstellagentView

import SwiftUI

struct AboutConstellagentView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .font(AppFont.body())
            .navigationTitle("About Constellagent")
    ...
    @ViewBuilder private var header: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text("Control **Constellagent** from your iPhone.")
            ...
            calloutCard(
                ...
                color: .cyan,
                text: "The Constellagent runtime stays on your device. Your phone is a secure remote control connected through a relay."
            )
        }
        .padding(.top, 8)
    }
    ...
    @ViewBuilder private var howItWorksSection: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            bulletList([
                ...
                "The bridge forwards it to `constellagent app-server` via JSON-RPC",
    ...
    @ViewBuilder private var architectureDiagram: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            VStack(spacing: 0) {
                diagramStep(from: "Constellagent iOS", to: "Bridge (Device)", via: "WebSocket")
                diagramStep(from: "Bridge (Device)", to: "constellagent app-server", via: "JSON-RPC")
                diagramStep(from: "constellagent app-server", to: "~/.constellagent/sessions", via: "JSONL rollout", isLast: true)
            }
            .padding(16)
            .background(
    ...
    private func diagramStep(from: String, to: String, via: String, isLast: Bool = false) -> some View {
        VStack(spacing: 0) {
            HStack(spacing: 10) {
                ...
                VStack(spacing: 2) {
                    ConstellagentIcon.image(systemName: "arrow.right")
    ...
    @ViewBuilder private var appServerSection: some View {
        VStack(alignment: .leading, spacing: 14) {
            sectionTitle("Constellagent App-Server")

            bodyText("The bridge spawns a **`constellagent app-server`** process — the same JSON-RPC interface behind the Constellagent desktop app and IDE extensions.")

            bulletList([
                "Phone conversations are first-class Constellagent sessions",
                "Produces JSONL rollout files under `~/.constellagent/sessions/`",
                "Threads started from your phone show up in Constellagent.app",
            ])

            calloutCard(
                icon: "point.topleft.down.to.point.bottomright.curvepath",
                color: .orange,
                text: "Already have a running Constellagent instance? Point the bridge at it with REMODEX_CODEX_ENDPOINT instead of spawning a new one."
            )
        }
    }
    ...
    @ViewBuilder private var resilienceSection: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            iconRow("cpu.fill", "Constellagent process stays alive across transient drops")
    ...
    @ViewBuilder private var desktopSection: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            bodyText("Threads from your phone are persisted as JSONL rollout files, so they appear in **Constellagent.app** on your device.")

            calloutCard(
                icon: "macbook.and.iphone",
                color: .blue,
                text: "The desktop app doesn't live-reload external writes. Use the desktop app handoff button in Constellagent to continue the current thread on your device."
            )
        }
    }
    ...
    private func iconRow(_ icon: String, _ text: String) -> some View {
        HStack(alignment: .top, spacing: 12) {
            ConstellagentIcon.image(systemName: icon)
