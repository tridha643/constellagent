...
// Depends on: SwiftUI, UIKit, ConstellagentBridgeUpdatePrompt

import SwiftUI
import UIKit

struct BridgeUpdateSheet: View {
    let prompt: ConstellagentBridgeUpdatePrompt
    let isRetrying: Bool
    let onRetry: () -> Void
                    ...
                    if let command = prompt.command, !command.isEmpty {
                        ...
                        HStack(spacing: 12) {
                            ...
                            Button {
                                ...
                                HStack(spacing: 6) {
                                    ConstellagentIcon.image(systemName: didCopyCommand ? "checkmark" : "doc.on.doc")
                        ...
                        Text("Install the latest Constellagent build on this iPhone, then come back here and reconnect.")
                            .font(AppFont.body())
                            .foregroundStyle(.secondary)
                    }
                }
