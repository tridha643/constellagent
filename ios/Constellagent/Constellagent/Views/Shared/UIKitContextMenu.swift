...
//          metric via `ConstellagentIcon.menuUIImage(systemName:)`, restoring
...
//   the call site, e.g. `SidebarThreadRowView(...).environment(constellagent)`.
...
extension View {
    ...
    /// metric (see `ConstellagentIcon.menuUIImage`).
    ///
