...
struct ComposerBottomBar: View {
    ...
    let selectedAccessMode: ConstellagentAccessMode
    let contextWindowUsage: ContextWindowUsage?
    let rateLimitBuckets: [ConstellagentRateLimitBucket]
    ...
    let onSelectAccessMode: (ConstellagentAccessMode) -> Void
    ...
    private var inlineAccessMenuLabel: some View {
        Menu {
            ForEach(ConstellagentAccessMode.allCases, id: \.rawValue) { mode in
                Button {
                    HapticFeedback.shared.triggerImpactFeedback(style: .light)
    ...
    private var attachmentMenu: some View {
        Menu {
            // `ConstellagentIcon.menuLabel` keeps Central artwork in SwiftUI Menus
            ...
            Toggle(isOn: Binding(
                ...
                ConstellagentIcon.menuLabel("Plan mode", systemName: "constellagent.plan-mode")
            }
            ...
            Section {
                Button {
                    ...
                    ConstellagentIcon.menuLabel("Photo library", systemName: "photo")
                }
                ...
                Button {
                    ...
                    ConstellagentIcon.menuLabel("Take a photo", systemName: "camera.fill")
                }
                .disabled(remainingAttachmentSlots == 0)
            }
        } label: {
    ...
    private func modelSupportsFastMode(_ model: ConstellagentModelOption) -> Bool {
    ...
    private var queueBadge: some View {
        HStack(spacing: 3) {
            if isQueuePaused {
                ConstellagentIcon.image(systemName: "pause.fill")
...
private struct ComposerRuntimeMenuControl: View, Equatable {
    ...
    private var compactModelTitle: String {
        ...
        let words = normalized.filter { word in
            ...
            return lowercased != "gpt" && lowercased != "constellagent"
        }
...
private struct AllModelsSheet: View {
    let models: [ConstellagentModelOption]
    let selectedModelID: String?
    let isLoadingModels: Bool
    let modelSupportsFastMode: (ConstellagentModelOption) -> Bool
            ...
            Group {
                if isLoadingModels {
                    ...
                    ContentUnavailableView {
                        ConstellagentIcon.label("No models available", systemName: "square.stack.3d.up.slash")
                    } description: {
                        Text("Reconnect to your local Constellagent bridge to refresh the model list.")
                    }
                } else {
                    List {
                        Section {
    ...
    @ViewBuilder
    private func modelRow(for model: ConstellagentModelOption) -> some View {
        let title = TurnComposerMetaMapper.modelTitle(for: model)
        HStack(alignment: .top, spacing: 12) {
            ConstellagentIcon.image(systemName: model.id == selectedModelID ? "checkmark.circle.fill" : "circle")
                .font(.system(size: 18))
            ...
            VStack(alignment: .leading, spacing: 2) {
                HStack(spacing: 6) {
                    ...
                    if modelSupportsFastMode(model) {
                        Image(systemName: ConstellagentServiceTier.fast.iconName)
