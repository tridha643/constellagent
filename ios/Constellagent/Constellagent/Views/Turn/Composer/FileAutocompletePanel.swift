...
struct FileAutocompletePanel: View {
    let items: [ConstellagentFuzzyFileMatch]
    var pluginItems: [ConstellagentPluginMetadata] = []
    let isLoading: Bool
    var isLoadingPlugins: Bool = false
    let query: String
    var pluginQuery: String = ""
    let onSelect: (ConstellagentFuzzyFileMatch) -> Void
    var onSelectPlugin: (ConstellagentPluginMetadata) -> Void = { _ in }
                    ...
                    LazyVStack(alignment: .leading, spacing: 0) {
                        ...
                        ForEach(pluginItems) { item in
                            Button {
                                ...
                                HStack(spacing: 8) {
                                    ConstellagentIcon.image(systemName: "circle.grid.2x2")
