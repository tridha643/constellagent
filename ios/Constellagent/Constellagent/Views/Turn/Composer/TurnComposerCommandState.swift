...
// Depends on: Foundation, ConstellagentReviewTarget
...
enum TurnComposerSlashCommand: String, Identifiable, Codable, Equatable, Sendable {
    ...
    var subtitle: String {
        ...
        case .feedback:
            return "Share feedback on Constellagent with the developer"
        case .fork:
            return "Fork this thread into local or a new worktree"
        case .status:
            return "Show context usage and rate limits"
        case .subagents:
            return "Insert a canned prompt that asks Constellagent to delegate work"
        }
    }
...
enum TurnComposerReviewTarget: String, Codable, Equatable, Sendable {
    ...
    var constellagentReviewTarget: ConstellagentReviewTarget {
        switch self {
        case .uncommittedChanges:
    ...
    // need to seed `ConstellagentPendingThreadComposerAction.codeReview` for a brand-new
    // thread without duplicating the case mapping.
    var constellagentPendingTarget: ConstellagentPendingCodeReviewTarget {
        switch self {
        case .uncommittedChanges:
            return .uncommittedChanges
        case .baseBranch:
