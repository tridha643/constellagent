...
// Layer: View
// Exports: TurnView
// Depends on: ConstellagentService, TurnViewModel, TurnConversationContainerView, TurnComposerHostView, TurnViewAlertModifier, TurnViewLifecycleModifier
...
struct TurnView: View {
    ...
    @State private var alertApprovalRequest: ConstellagentApprovalRequest?
    ...
    @State private var voiceRecoveryReason: ConstellagentVoiceFailureReason?
    ...
    var body: some View {
        ...
        let timelineState = constellagent.timelineState(for: thread.id)
        let renderSnapshot = timelineState.renderSnapshot
        let activeTurnID = renderSnapshot.activeTurnID
        let planSessionSource = constellagent.currentPlanSessionSource(for: thread.id)
        ...
        let threadDisplayPhase = constellagent.threadDisplayPhase(
            threadId: thread.id,
        ...
        let showsGitControls = constellagent.isConnected && gitWorkingDirectory != nil
        ...
        let onTapMacHandoff: (() -> Void)? = constellagent.isConnected && constellagent.supportsDesktopAppHandoff ? {
        ...
        let onTapNewChat: (() -> Void)? = constellagent.isConnected && !isWorktreeProject ? {
            startSiblingChat()
        } : nil
        ...
        // Closures extracted from the constructor call to keep the Swift type
        // checker from blowing up on the giant TurnConversationContainerView
        // expression. Inline closures stacked into a single call exceed the
        // solver budget — typed lets keep each one local.
        let threadID = thread.id
        let onReportError: (String) -> Void = { [constellagent, threadID] errorMessage in
            openURL(AppEnvironment.feedbackMailtoURL(
                errorMessage: errorMessage,
                threadId: threadID,
                isConnected: constellagent.isConnected,
                cliVersion: constellagent.bridgeInstalledVersion
            ))
        }
        let onDismissError: () -> Void = { [constellagent] in
            constellagent.lastErrorMessage = nil
        }
        let onRetryUserMessage: (String) -> Void = { [constellagent, threadID] messageText in
            viewModel.input = messageText
            viewModel.saveLocalDraft(constellagent: constellagent, threadID: threadID)
            isInputFocused = true
        }
        let onTapAssistantRevert: (ConstellagentMessage) -> Void = { message in
            startAssistantRevertPreview(message: message, gitWorkingDirectory: gitWorkingDirectory)
        }
        let onTapSubagent: (ConstellagentSubagentThreadPresentation) -> Void = { subagent in
            openThread(subagent.threadId)
        }
        let onRevealEarlierMessages: (Int) -> Void = { [constellagent, threadID] pageSize in
            constellagent.noteThreadHistoryRevealRequested(threadId: threadID, pageSize: pageSize)
        }
        let onLoadRemoteEarlierMessages: () -> Void = { [constellagent, threadID] in
            Task { @MainActor in
                await constellagent.loadOlderThreadHistoryPage(threadId: threadID)
            }
        }
        let onRetryEarlierMessages: (@escaping () -> Void) -> Void = { [constellagent, threadID] completion in
            Task { @MainActor in
                defer { completion() }
                _ = try? await constellagent.loadThreadHistoryIfNeeded(threadId: threadID, forceRefresh: true)
            }
        }
        ...
        let composerView: AnyView = AnyView(composerWithSubagentAccessory(
        ...
        let structuredPromptReplacementComposer: (ConstellagentMessage) -> AnyView = { message in
            AnyView(composerStructuredPromptReplacement(message: message))
        }
        let emptyToastOverlay: AnyView = AnyView(EmptyView())
        ...
        return TurnConversationContainerView(
                ...
                structuredPromptReplacementComposer: structuredPromptReplacementComposer,
                repositoryLoadingToastOverlay: emptyToastOverlay,
                usageToastOverlay: emptyToastOverlay,
        ...
        .toolbar {
            TurnToolbarContent(
                ...
                showsThreadActions: constellagent.isConnected,
        ...
        .fullScreenCover(isPresented: isCameraPresentedBinding) {
            CameraImagePicker { data in
                viewModel.enqueueCapturedImageData(data, constellagent: constellagent, threadID: thread.id)
            }
            .ignoresSafeArea()
        }
        ...
        .turnViewLifecycle(
            ...
            onPhotoPickerItemsChanged: { newItems in
                ...
                DispatchQueue.main.async { [viewModel] in
                    viewModel.enqueuePhotoPickerItems(newItems, constellagent: constellagent, threadID: thread.id)
            ...
            onThreadRunningChanged: { wasRunning, isRunning in
                ...
                // to avoid AttributeGraph cycles when the parent re-renders.
                DispatchQueue.main.async { [viewModel] in
                    viewModel.flushQueueIfPossible(constellagent: constellagent, threadID: thread.id)
            ...
            onScenePhaseChanged: { phase in
                ...
                // to avoid AttributeGraph cycles when the parent re-renders.
                DispatchQueue.main.async { [viewModel] in
                    viewModel.saveLocalDraft(constellagent: constellagent, threadID: thread.id, persistToDisk: true)
                }
        ...
        .onChange(of: renderSnapshot.timelineChangeToken) { _, _ in
            ...
            DispatchQueue.main.async { [viewModel] in
                viewModel.reconcileDismissedStructuredPlanPrompts(messages: messages, constellagent: constellagent)
            }
        }
            ...
            if let context = threadNavigationContext(for: resolvedThread) {
                TurnThreadPathSheet(
                    ...
                    onRenameThread: { newName in
                        constellagent.renameThread(thread.id, name: newName)
                    }
                )
            }
        }
        ...
        .turnViewAlerts(
            ...
            onDeclineApproval: { request in
                viewModel.decline(request, constellagent: constellagent) { didSucceed in
                    if didSucceed {
                        syncApprovalAlertPresentation()
                    } else {
            ...
            onApproveApproval: { request in
                viewModel.approve(request, constellagent: constellagent) { didSucceed in
                    if didSucceed {
                        syncApprovalAlertPresentation()
                    } else {
                        restoreApprovalAlert(afterFailureOf: request)
            ...
            onConfirmGitSyncAction: { alertAction in
                viewModel.confirmGitSyncAlertAction(
                    ...
                    workingDirectory: gitWorkingDirectory,
                    threadID: thread.id,
                    activeTurnID: constellagent.activeTurnID(for: thread.id)
                )
            },
    ...
    private var timelineFooterErrorMessage: String? {
        TurnFooterErrorFilter.visibleFooterMessage(from: constellagent.lastErrorMessage)
    }
    ...
    private var voiceRecoveryPresentation: VoiceRecoveryPresentation? {
        ...
        guard let resolvedReason = constellagent.resolveVoiceRecoveryReason(voiceRecoveryReason) else {
            return nil
        }
    ...
    private var connectionRecoverySnapshot: ConnectionRecoverySnapshot? {
        TurnConnectionRecoverySnapshotBuilder.makeSnapshot(
            ...
            isConnecting: constellagent.isConnecting,
            shouldAutoReconnectOnForeground: constellagent.shouldAutoReconnectOnForeground,
            isRetryingConnectionRecovery: isRetryingConnectionRecovery,
            lastErrorMessage: constellagent.lastErrorMessage
        )
    }

    private var canWakeSavedMacDisplay: Bool {
        constellagent.canWakePreferredMacDisplay
    }
    ...
    private func startSiblingChat() {
        Task { @MainActor in
            ...
            do {
                _ = try await constellagent.startThreadIfReady(preferredProjectPath: resolvedProjectPathForFollowUpThread())
            } catch {
                if let message = constellagent.userFacingTurnErrorMessageForFooter(from: error),
                   constellagent.lastErrorMessage?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ?? true {
    ...
    private func handleSend() {
        ...
        viewModel.clearComposerAutocomplete()
        viewModel.sendTurn(constellagent: constellagent, subscriptions: subscriptions, threadID: thread.id)
        isInputFocused = false
    }

    @ViewBuilder
    private func composerStructuredPromptReplacement(message: ConstellagentMessage) -> some View {
        if let request = message.structuredUserInputRequest {
            let isDismissed = viewModel.isStructuredPlanPromptDismissed(request.requestID, constellagent: constellagent)
            let isDismissing = viewModel.isStructuredPlanPromptDismissing(request.requestID, constellagent: constellagent)
            ...
            if !isDismissed {
                StructuredUserInputCard(
                    ...
                    onSecondaryAction: isDismissing ? nil : {
                        isInputFocused = true
                        viewModel.dismissStructuredPlanPrompt(message, constellagent: constellagent, threadID: thread.id)
                    }
                )
                ...
                composerWithSubagentAccessory(
                    ...
                    activeTurnID: constellagent.activeTurnID(for: thread.id),
                    isThreadRunning: constellagent.timelineState(for: thread.id).renderSnapshot.isThreadRunning,
                    isEmptyThread: constellagent.timelineState(for: thread.id).renderSnapshot.messages.isEmpty,
                    isWorktreeProject: currentResolvedThread.isManagedWorktreeProject,
                    showsGitControls: constellagent.isConnected && currentResolvedThread.gitWorkingDirectory != nil,
    ...
    private var currentResolvedThread: ConstellagentThread {
        constellagent.thread(for: thread.id) ?? thread
    }
    ...
    private func handleWorktreeHandoffTap(currentThread: ConstellagentThread) {
        ...
        guard let associatedWorktreePath = constellagent.associatedManagedWorktreePath(for: thread.id) else {
    ...
    private func handleInitialAppear(activeTurnID: String?) {
        ...
        if let pendingComposerAction = constellagent.consumePendingComposerAction(for: thread.id) {
            ...
            viewModel.restoreSavedLocalDraftIfNeeded(constellagent: constellagent, threadID: thread.id)
        }
    }
    ...
    private func startAssistantRevertPreview(message: ConstellagentMessage, gitWorkingDirectory: String?) {
        guard let gitWorkingDirectory,
              let changeSet = constellagent.readyChangeSet(forAssistantMessage: message),
              let presentation = constellagent.assistantRevertPresentation(
                for: message,
                workingDirectory: gitWorkingDirectory
              ),
        ...
        Task { @MainActor in
            do {
                let preview = try await constellagent.previewRevert(
                    changeSet: changeSet,
                    workingDirectory: gitWorkingDirectory
                )
    ...
    private func confirmAssistantRevert(gitWorkingDirectory: String?) {
        ...
        Task { @MainActor in
            do {
                let applyResult = try await constellagent.applyRevert(
                    changeSet: changeSet,
                    workingDirectory: gitWorkingDirectory
                )
    ...
    private func prepareThreadIfReady(gitWorkingDirectory: String?) async {
        let didPrepare = await constellagent.prepareThreadForDisplay(threadId: thread.id)
        guard didPrepare, !Task.isCancelled, constellagent.activeThreadId == thread.id else { return }
        await constellagent.refreshContextWindowUsage(threadId: thread.id)
        guard !Task.isCancelled, constellagent.activeThreadId == thread.id else { return }
        viewModel.flushQueueIfPossible(constellagent: constellagent, threadID: thread.id)
        guard !Task.isCancelled, constellagent.activeThreadId == thread.id else { return }
        ...
        viewModel.requestCreateGitWorktree(
            ...
            onOpenWorktree: { result in
                ...
                Task { @MainActor in
                    ...
                    do {
                        let forkedThread = try await constellagent.forkThreadIfReady(
                            from: thread.id,
    ...
    private func resolvedProjectPathForFollowUpThread() -> String? {
        let currentThread = constellagent.thread(for: thread.id) ?? thread
    ...
    private func startCodeReviewThread(target: TurnComposerReviewTarget) {
        Task { @MainActor in
            do {
                _ = try await constellagent.startThreadIfReady(
                    preferredProjectPath: resolvedProjectPathForFollowUpThread(),
                    pendingComposerAction: .codeReview(target: target.constellagentPendingTarget)
                )
    ...
    private var orderedModelOptions: [ConstellagentModelOption] {
    ...
    private var selectedModelTitle: String {
        if let selectedModel = constellagent.selectedModelOption() {
            return TurnComposerMetaMapper.modelTitle(for: selectedModel)
        }

        return TurnComposerMetaMapper.modelTitle(forIdentifier: constellagent.selectedModelId)
    }

    private var approvalForThread: ConstellagentApprovalRequest? {
        constellagent.pendingApproval(for: thread.id)
    }
    ...
    private func restoreApprovalAlert(afterFailureOf request: ConstellagentApprovalRequest) {
        alertApprovalRequest = approvalForThread ?? request
        isApprovalAlertPresented = alertApprovalRequest != nil
    }

    private var parentThread: ConstellagentThread? {
        guard let parentThreadId = thread.parentThreadId else {
            return nil
        }

        return constellagent.thread(for: parentThreadId)
    }

    private func threadNavigationContext(for thread: ConstellagentThread) -> TurnThreadNavigationContext? {
    ...
    private func composerWithSubagentAccessory(
        currentThread: ConstellagentThread,
        activeTurnID: String?,
        isThreadRunning: Bool,
        isEmptyThread: Bool,
        ...
        VStack(spacing: 8) {
            if let parentThread = parentThread {
                SubagentParentAccessoryCard(
                    ...
                    agentLabel: constellagent.resolvedSubagentDisplayLabel(threadId: thread.id, agentId: thread.agentId)
                        ?? "Subagent",
            ...
            TurnComposerHostView(
                ...
                onSelectGitBranch: { branch in
                    ...
                    if let worktreePath = viewModel.worktreePathForCheckedOutElsewhereBranch(branch) {
                        if let normalizedWorktreePath = ConstellagentThreadStartProjectBinding.normalizedProjectPath(worktreePath) {
                ...
                onOpenFeedbackMail: {
                    openURL(AppEnvironment.feedbackMailtoURL(
                        errorMessage: constellagent.lastErrorMessage,
                        threadId: thread.id,
                        isConnected: constellagent.isConnected,
                        cliVersion: constellagent.bridgeInstalledVersion
                    ))
    ...
    private func stopVoiceTranscription(operationGeneration: Int) async {
        ...
        do {
            ...
            isVoiceRecording = false
            voiceTranscriptionManager.resetMeteringState()
            let transcript = try await constellagent.transcribeVoiceAudioFile(
                at: clip.url,
    ...
    private func startVoiceRecordingIfReady() async {
        ...
        guard constellagent.supportsBridgeVoiceTranscription else {
    ...
    private var voiceAutoStopThreshold: TimeInterval {
        max(0, ConstellagentVoiceTranscriptionPreflight.maxDurationSeconds - 0.25)
    }
    ...
    private func presentVoiceRecovery(for error: Error) {
        presentVoiceRecovery(for: constellagent.classifyVoiceFailure(error))
    }

    private func presentVoiceRecovery(for reason: ConstellagentVoiceFailureReason) {
    ...
    // Opens the Constellagent ChatGPT login flow on the paired Mac, then lets account sync update voice readiness.
    private func startVoiceLoginOnMac() {
        Task { @MainActor in
            do {
                try await constellagent.startOrResumeGPTLoginOnMac()
    ...
    private func openThread(_ threadId: String) {
        constellagent.activeThreadId = threadId
        constellagent.markThreadAsViewed(threadId)
        constellagent.requestImmediateActiveThreadSync(threadId: threadId, forceHistoryRefresh: true)
    }
    ...
    private func resolvedEmptyState(for phase: ConstellagentService.ThreadDisplayPhase) -> AnyView {
        switch phase {
        case .loading:
...
#Preview {
    NavigationStack {
        TurnView(
            thread: ConstellagentThread(id: "thread_preview", title: "Preview"),
