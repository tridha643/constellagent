...
// Depends on: SwiftUI, ConstellagentApprovalRequest, GitActionModels

import SwiftUI

private struct TurnViewAlertModifier: ViewModifier {
    @Binding var alertApprovalRequest: ConstellagentApprovalRequest?
    ...
    let onDeclineApproval: (ConstellagentApprovalRequest) -> Void
    let onApproveApproval: (ConstellagentApprovalRequest) -> Void
    ...
    func body(content: Content) -> some View {
        content
            ...
            .alert("Continue on Desktop App", isPresented: $isShowingMacHandoffConfirm) {
                ...
                Text("Constellagent will force close and reopen Constellagent.app on this device. Any desktop runs in progress will be stopped, and unsaved draft text there may be lost before this chat is opened.")
            }
            .alert(
    ...
    private func approvalAlertMessage(for request: ConstellagentApprovalRequest) -> String {
        var lines: [String] = []
        ...
        if lines.isEmpty {
            return "Constellagent is requesting permission to continue."
        }
...
extension View {
    func turnViewAlerts(
        alertApprovalRequest: Binding<ConstellagentApprovalRequest?>,
        ...
        onDeclineApproval: @escaping (ConstellagentApprovalRequest) -> Void,
        onApproveApproval: @escaping (ConstellagentApprovalRequest) -> Void,
