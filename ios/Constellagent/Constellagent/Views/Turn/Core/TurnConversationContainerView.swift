...
struct TurnConversationContainerView: View {
    ...
    let planSessionSource: ConstellagentPlanSessionSource?
    let allowsAssistantPlanFallbackRecovery: Bool
    let threadMessagesForPlanMatching: [ConstellagentMessage]
    ...
    let structuredPromptReplacementComposer: ((ConstellagentMessage) -> AnyView)?
    let repositoryLoadingToastOverlay: AnyView
    let usageToastOverlay: AnyView
    let isRepositoryLoadingToastVisible: Bool
    let onRetryUserMessage: (String) -> Void
    let onTapAssistantRevert: (ConstellagentMessage) -> Void
    let onTapSubagent: (ConstellagentSubagentThreadPresentation) -> Void
        ...
        if let pinnedTaskPlanMessage = messageLayout.pinnedTaskPlanMessage {
            ...
            return AnyView(
                AccessoryBackedEmptyState(
                    ...
                    summary: summary.isEmpty ? "Constellagent has prepared a plan for this chat." : summary,
    ...
    private static func buildMessageLayout(
        ...
        planSessionSource: ConstellagentPlanSessionSource?
    ) -> TimelineMessageLayout {
        var timelineMessages: [ConstellagentMessage] = []
        timelineMessages.reserveCapacity(messages.count)
        var pinnedTaskPlanMessage: ConstellagentMessage?
        var activeStructuredPromptMessage: ConstellagentMessage?
...
private struct TimelineMessageLayout: Equatable {
    let timelineMessages: [ConstellagentMessage]
    let pinnedTaskPlanMessage: ConstellagentMessage?
    let activeStructuredPromptMessage: ConstellagentMessage?
...
extension ConstellagentMessage {
