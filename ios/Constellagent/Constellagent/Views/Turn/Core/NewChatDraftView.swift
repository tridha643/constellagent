...
// Depends on: SwiftUI, PhotosUI, ConstellagentService, TurnComposerHostView,
...
struct NewChatDraftView: View {
    ...
    let onOpenThread: @MainActor @Sendable (ConstellagentThread) -> Void
    ...
    var body: some View {
        ...
        .onChange(of: constellagent.isConnected) { wasConnected, isConnected in
            if !isConnected {
                return
            }
        ...
        .onChange(of: scenePhase) { _, phase in
            ...
            viewModel.saveLocalDraft(constellagent: constellagent, threadID: route.id, persistToDisk: true)
        }
        ...
        .fullScreenCover(isPresented: isCameraPresentedBinding) {
            CameraImagePicker { data in
                viewModel.enqueueCapturedImageData(data, constellagent: constellagent, threadID: route.id)
            }
            .ignoresSafeArea()
        }
        .photosPicker(
        ...
        .onChange(of: viewModel.photoPickerItems) { _, newItems in
            ...
            // to avoid AttributeGraph cycles when the parent re-renders.
            DispatchQueue.main.async { [viewModel] in
                viewModel.enqueuePhotoPickerItems(newItems, constellagent: constellagent, threadID: route.id)
    ...
    private var pendingDraftUserMessage: ConstellagentMessage? {
        constellagent.messages(for: route.id).last { message in
            message.role == .user && message.deliveryState == .pending
        }
    }

    private func pendingDraftUserMessageView(_ message: ConstellagentMessage) -> some View {
        VStack(spacing: 0) {
            UserMessageBubble(
                message: message,
                text: message.text,
    ...
    private func refreshDraftGitStateIfNeeded() {
        guard hasSelectedProject, constellagent.isConnected else {
            return
        }
    ...
    private var showsGeneralFolderPicker: Bool {
        isFromGeneralChat
            && ConstellagentThreadStartProjectBinding.normalizedProjectPath(route.preferredProjectPath) != nil
    }
    ...
    private func presentRepositoryDiff() {
        ...
        Task { @MainActor in
            ...
            let gitService = GitActionsService(constellagent: constellagent, workingDirectory: selectedProjectPath)
            do {
    ...
    private func folderPickerMenu() -> UIMenu {
        guard !projectChoices.isEmpty else {
            return UIMenu(children: [
                UIAction(
                    ...
                    image: ConstellagentIcon.menuUIImage(systemName: "folder"),
                    attributes: [.disabled]
                ) { _ in },
            ])
        }
        ...
        let actions = projectChoices.map { choice in
            UIAction(
                title: choice.label,
                image: ConstellagentIcon.menuUIImage(systemName: choice.iconSystemName),
    ...
    // Uses the custom Constellagent folder glyph so the inline picker matches the rest of the sidebar.
    ...
    private var trustedHostName: String? {
        let trimmed = (constellagent.trustedPairPresentation?.name ?? "")
        ...
        VStack(spacing: 8) {
            ...
            TurnComposerHostView(
                ...
                onStartCodeReviewThread: { target in
                    viewModel.applyPendingComposerAction(.codeReview(target: target.constellagentPendingTarget))
                },
    ...
    private var draftThread: ConstellagentThread {
        ConstellagentThread(
            id: route.id,
            title: "New thread",
    ...
    private var voiceRecoveryPresentation: VoiceRecoveryPresentation? {
        guard let voiceRecoveryReason,
              let resolvedReason = constellagent.resolveVoiceRecoveryReason(voiceRecoveryReason) else {
            return nil
        }
    ...
    private func initializeProjectSelectionIfNeeded() {
        ...
        selectedProjectPath = ConstellagentThreadStartProjectBinding.normalizedProjectPath(route.preferredProjectPath)
    ...
    private func sendDraft() {
        ...
        let openThread: @MainActor @Sendable (ConstellagentThread) -> Void = { thread in
            onOpenThread(thread)
        }
        Task { @MainActor in
            await Task.yield()
    ...
    // Opens the Constellagent ChatGPT login flow on the paired Mac while the draft composer stays in place.
    ...
    NavigationStack {
        NewChatDraftView(
            route: NewChatDraftRoute(
                id: "draft_preview",
                preferredProjectPath: "/Users/emanueledipietro/Developer/Constellagent",
                source: .generalChat
            ),
            onOpenThread: { _ in }
        )
