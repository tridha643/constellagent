...
// Depends on: SwiftUI, UIKit, ConstellagentService
    ...
    var body: some View {
        GlassAccessoryCard(onTap: onTap) {
            ZStack {
                ...
                ConstellagentIcon.image(systemName: "arrow.turn.up.left")
...
struct RuntimeDebugLogSheet: View {
    ...
    private var combinedLogText: String {
        constellagent.runtimeDebugLogEntries.joined(separator: "\n")
    }

    var body: some View {
        NavigationStack {
            Group {
                if constellagent.runtimeDebugLogEntries.isEmpty {
                    ContentUnavailableView {
                        ConstellagentIcon.label("No Runtime Logs Yet", systemName: "list.bullet.rectangle")
                    } description: {
            ...
            .toolbar {
                ...
                ToolbarItemGroup(placement: .topBarTrailing) {
                    Button("Clear") {
                        constellagent.clearRuntimeDebugLog()
                    }
