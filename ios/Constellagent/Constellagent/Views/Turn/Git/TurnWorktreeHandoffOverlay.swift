...
// Depends on: SwiftUI, ConstellagentWorktreeIcon
...
enum TurnWorktreeOverlayMode {
    ...
    var message: String {
        switch self {
        case .handoff:
            return "Create and check out a branch in a new worktree to continue in parallel. The branch is normalized with the constellagent/ prefix."
        case .fork:
...
struct TurnWorktreeHandoffOverlay: View {
    ...
    private var normalizedBranchName: String {
        constellagentNormalizedCreatedBranchName(branchName)
    }
    ...
    private var header: some View {
        HStack(alignment: .top) {
            ZStack {
                ...
                ConstellagentWorktreeIcon(pointSize: 16, weight: .semibold)
            ...
            Button(action: onClose) {
                ConstellagentIcon.image(systemName: "xmark")
