...
extension TurnGitActionKind {
    ...
    private static func resizedSymbol(named name: String, size: CGSize) -> UIImage {
        ...
        guard let symbol = ConstellagentIcon.uiImage(systemName: name, withConfiguration: config)?.withRenderingMode(.alwaysTemplate) else {
            return UIImage()
        }
        let renderer = UIGraphicsImageRenderer(size: size)
...
private struct UIKitGitActionsMenuButton: UIViewRepresentable {
    ...
    final class Coordinator {
        ...
        private func makePlaceholderChangesRow(title: String, systemImage: String) -> UIAction {
            return UIAction(
                title: title,
                image: ConstellagentIcon.menuUIImage(systemName: systemImage),
                attributes: .disabled,
                handler: { _ in }
            )
        }
        ...
        private func makeChangesAction(totals: GitDiffTotals) -> UIAction {
            ...
            let action = UIAction(
                ...
                image: ConstellagentIcon.menuUIImage(systemName: "doc.text.magnifyingglass"),
