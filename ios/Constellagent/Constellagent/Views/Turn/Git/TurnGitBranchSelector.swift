...
func constellagentNormalizedCreatedBranchName(_ rawName: String) -> String {
    let trimmedName = rawName.trimmingCharacters(in: .whitespacesAndNewlines)
    guard !trimmedName.isEmpty else { return "" }
    if trimmedName.hasPrefix("constellagent/") {
        return trimmedName
    }
    return "constellagent/\(trimmedName)"
}

// Leaves "open elsewhere" branches selectable so the caller can surface the right alert or git error.
func constellagentCurrentBranchSelectionIsDisabled(
    branch: String,
...
func constellagentSelectableDefaultBranch(
...
struct TurnGitBranchSelector: View, Equatable {
    ...
    private var localDefaultBranch: String? {
        ...
        return constellagentSelectableDefaultBranch(
    ...
    var body: some View {
        Button {
            ...
            ComposerPillLabel(
                ...
                iconSystemName: "constellagent.git-branch",
...
struct TurnGitBranchPickerSheet: View {
    ...
    private var isNewBranchNameValid: Bool {
        ...
        guard !trimmed.isEmpty, trimmed != "constellagent/" else { return false }
        return true
    }
    ...
    private var suggestedCreateBranchName: String? {
        ...
        let candidate = constellagentNormalizedCreatedBranchName(searchText)
    ...
    var body: some View {
        List {
            Section(sectionTitle) {
                if showsDefaultBranchRow, let defaultBranch {
                    let isDisabled = constellagentCurrentBranchSelectionIsDisabled(
                ...
                if orderedBranches.isEmpty {
                    ContentUnavailableView {
                        ConstellagentIcon.label("No branches found", systemName: "arrow.triangle.branch")
                    } description: {
            ...
            if allowsSelectingCurrentBranch {
                Section {
                    ...
                    Button {
                        let fromSearch = constellagentNormalizedCreatedBranchName(searchText)
                        newBranchName = fromSearch.isEmpty ? "constellagent/" : fromSearch
        ...
        .alert("New branch", isPresented: $isShowingCreateBranchPrompt) {
            TextField("constellagent/my-feature", text: $newBranchName)
            Button("Cancel", role: .cancel) {
                newBranchName = ""
            }
            Button("Create") {
                let branchName = constellagentNormalizedCreatedBranchName(newBranchName)
...
#Preview("Branch Picker") {
    TurnGitBranchPickerSheet(
        branches: [
            "feature/auth-flow",
            "feature/dark-mode",
            "constellagent/onboarding-v2",
            "constellagent/sidebar-refactor",
            "fix/crash-on-launch",
        ...
        selectedBranch: "constellagent/onboarding-v2",
        defaultBranch: "main",
        currentBranch: "constellagent/onboarding-v2",
