...
// Depends on: SwiftUI, ConstellagentService, ConstellagentMessage, StructuredUserInputCardView
...
struct PlanSystemCard: View {
    ...
    private var threadMessages: [ConstellagentMessage] {
        constellagent.messages(for: message.threadId)
    }
    ...
    private var rawInferredQuestionnaire: InferredPlanQuestionnaire? {
        resolvedInferredPlanQuestionnaire(
            ...
            shouldRecoverFallback: constellagent.allowsInferredPlanQuestionnaireFallback(for: message.threadId),
...
func resolvedInferredPlanQuestionnaire(
    ...
    threadMessages: [ConstellagentMessage],
...
private func normalizedQuestionSignature(
    for questions: [ConstellagentStructuredUserInputQuestion]
...
struct InferredPlanQuestionnaireCard: View {
    ...
    private func submitAnswers(_ answersByQuestionID: [String: [String]]) {
        ...
        Task { @MainActor in
            do {
                try await constellagent.submitInferredPlanQuestionnaireResponse(
...
struct ProposedPlanResultCard: View {
    ...
    let threadId: String
    let proposedPlan: ConstellagentProposedPlan
            ...
            HStack(alignment: .center, spacing: 8) {
                ...
                Button {
                    ...
                    HStack(spacing: 5) {
                        ...
                        ConstellagentIcon.image(systemName: isPlanExpanded ? "chevron.up" : "chevron.down")
    ...
    private func implementPlan() {
        ...
        Task { @MainActor in
            do {
                try await constellagent.implementProposedPlan(
                    threadId: threadId,
            ...
            HStack(spacing: 6) {
                Group {
                    if isLoading {
                        ...
                        ConstellagentIcon.image(systemName: systemName)
...
struct StructuredUserInputCard: View {
    ...
    let request: ConstellagentStructuredUserInputRequest
    ...
    private func submitAnswers(_ answersByQuestionID: [String: [String]]) {
        ...
        Task { @MainActor in
            do {
                try await constellagent.respondToStructuredUserInput(
            ...
            PlanModeCardContainer(title: "Input needed", showsProgress: false) {
                HStack(alignment: .center, spacing: 12) {
                    VStack(alignment: .leading, spacing: 4) {
                        Text(questionCount == 1 ? "Constellagent needs one answer" : "Constellagent needs \(questionCount) answers")
                    ...
                    ConstellagentIcon.image(systemName: "chevron.up.circle.fill")
...
struct StructuredUserInputSheet: View {
    ...
    let requestMessage: ConstellagentMessage
    let planMessage: ConstellagentMessage?
...
private struct PlanStepList: View {
    let steps: [ConstellagentPlanStep]
...
private struct PlanStepRow: View {
    let step: ConstellagentPlanStep

    var body: some View {
        HStack(alignment: .top, spacing: 10) {
            ConstellagentIcon.image(systemName: statusSymbol)
...
nonisolated enum InferredPlanQuestionnaireParser {
    ...
    private static func parseQuestionBlock(_ block: QuestionBlock) -> ParsedQuestionBlock {
        ...
        let explicitOptions = optionLines.map { optionLine in
            ConstellagentStructuredUserInputOption(label: optionLine, description: "")
        }
        ...
        let question = ConstellagentStructuredUserInputQuestion(
        ...
        return InferredPlanQuestionnaire(
            ...
            questions: [
                ConstellagentStructuredUserInputQuestion(
                    ...
                    header: "Next step",
                    question: "What should Constellagent produce next?",
                    isOther: false,
                    isSecret: false,
                    options: options
                ),
    ...
    private static func makeChoiceListOption(from block: QuestionBlock) -> ConstellagentStructuredUserInputOption? {
        ...
        return ConstellagentStructuredUserInputOption(
            label: firstLine,
    ...
    private static func inferredInlineOptions(
        from prompt: String
    ) -> (question: String, options: [ConstellagentStructuredUserInputOption])? {
        ...
        let options = optionLabels.map { label in
            ConstellagentStructuredUserInputOption(label: label, description: "")
        }
