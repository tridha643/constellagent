...
// Depends on: SwiftUI, ConstellagentMessage, ConstellagentCollaboration
...
struct PlanAccessorySnapshot: Equatable {
    ...
    let stepStatuses: [ConstellagentPlanStepStatus]
    ...
    init(
        ...
        stepStatuses: [ConstellagentPlanStepStatus] = []
    ) {
        self.title = title
    ...
    init(message: ConstellagentMessage) {
    ...
    private static func resolveStatus(
        from steps: [ConstellagentPlanStep],
        completedStepCount: Int
    ) -> PlanAccessoryStatus {
    ...
    private static func resolveSummary(from message: ConstellagentMessage, steps: [ConstellagentPlanStep]) -> String {
...
struct PlanAccessoryCard: View {
    ...
    private func stepTint(for status: ConstellagentPlanStepStatus) -> Color {
        switch status {
        case .pending:
...
enum PlanAccessoryPreviewFixtures {
    ...
    static let activeMessage = ConstellagentMessage(
        threadId: threadID,
        role: .system,
        kind: .plan,
        ...
        planState: ConstellagentPlanState(
            ...
            steps: [
                ConstellagentPlanStep(step: "Inspect the current conversation layout and top overlay behavior", status: .completed),
                ConstellagentPlanStep(step: "Move the active plan out of the timeline overlay and into a compact accessory", status: .inProgress),
                ConstellagentPlanStep(step: "Open the full task list in a sheet when the compact row is tapped", status: .pending),
            ]
        )
    )

    static let pendingMessage = ConstellagentMessage(
        threadId: threadID,
        role: .system,
        kind: .plan,
        text: "Planning...",
        planState: ConstellagentPlanState(
            ...
            steps: [
                ConstellagentPlanStep(step: "Confirm the runtime contract for plan updates", status: .pending),
                ConstellagentPlanStep(step: "Split the accessory into a reusable component", status: .pending),
                ConstellagentPlanStep(step: "Add focused previews for visual iteration", status: .pending),
            ]
        )
    )

    static let completedMessage = ConstellagentMessage(
        threadId: threadID,
        role: .system,
        kind: .plan,
        ...
        planState: ConstellagentPlanState(
            ...
            steps: [
                ConstellagentPlanStep(step: "Review the old overlay behavior", status: .completed),
                ConstellagentPlanStep(step: "Replace it with a compact accessory above the composer", status: .completed),
                ConstellagentPlanStep(step: "Present the full plan inside a sheet", status: .completed),
            ]
        )
    )
}
...
private struct PlanAccessoryInContextPreview: View {
    ...
    private var previewComposer: some View {
        HStack(spacing: 12) {
            ConstellagentIcon.image(systemName: "plus")
                .font(AppFont.system(size: 14, weight: .semibold))
                .foregroundStyle(.secondary)
                .frame(width: 32, height: 32)
                .background(Color(.secondarySystemBackground), in: Circle())

            Text("Ask Constellagent to continue...")
                .font(AppFont.body())
                .foregroundStyle(.secondary)

            Spacer(minLength: 0)

            ConstellagentIcon.image(systemName: "arrow.up.circle.fill")
