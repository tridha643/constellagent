...
// Depends on: SwiftUI, ConstellagentService, ConstellagentSubagentAction, ConstellagentSubagentThreadPresentation, AppFont
...
struct SubagentActionCard: View {
    ...
    let action: ConstellagentSubagentAction
    let onOpenSubagent: ((ConstellagentSubagentThreadPresentation) -> Void)?

    @Environment(ConstellagentService.self) private var constellagent
    @State private var isExpanded = true
    @State private var selectedAgentDetails: ConstellagentSubagentThreadPresentation?
    ...
    var body: some View {
        ...
        .sheet(item: $selectedAgentDetails) { agent in
            let resolved = constellagent.resolvedSubagentPresentation(agent, parentThreadId: parentThreadId)
    ...
    private var headerRow: some View {
        HStack(spacing: 8) {
            ...
            ConstellagentIcon.image(systemName: isExpanded ? "chevron.down" : "chevron.right")
    ...
    @ViewBuilder
    private func agentRowView(_ agent: ConstellagentSubagentThreadPresentation) -> some View {
        let resolved = constellagent.resolvedSubagentPresentation(agent, parentThreadId: parentThreadId)
        let title = resolvedAgentTitle(for: resolved)
    ...
    private func resolvedAgentTitle(for agent: ConstellagentSubagentThreadPresentation) -> String {
        constellagent.resolvedSubagentDisplayLabel(threadId: agent.threadId, agentId: agent.agentId)
            ?? agent.displayLabel
    }

    private func resolvedModelLabel(
        for agent: ConstellagentSubagentThreadPresentation,
    ...
    private func resolvedStatus(for agent: ConstellagentSubagentThreadPresentation) -> SubagentStatusPresentation {
        if constellagent.runningThreadIDs.contains(agent.threadId) || constellagent.activeTurnIdByThread[agent.threadId] != nil {
            return SubagentStatusPresentation(rawStatus: "running")
        }
        if let terminal = constellagent.latestTurnTerminalStateByThread[agent.threadId] {
            switch terminal {
    ...
    private func observedThread(for agent: ConstellagentSubagentThreadPresentation) -> ConstellagentThread? {
        constellagent.threads.first(where: { $0.id == agent.threadId })
    }
    ...
    private func detailText(for agent: ConstellagentSubagentThreadPresentation) -> String? {
        var sections: [String] = []
    ...
    private func hydrateChildThreadMetadata() async {
        await constellagent.loadSubagentThreadMetadataIfNeeded(threadIds: action.agentRows.map(\.threadId))
    }
}
        ...
        HStack(alignment: .center, spacing: 10) {
            ...
            if showsDetails, let onShowDetails {
                Button(action: onShowDetails) {
                    ConstellagentIcon.image(systemName: "info.circle")
