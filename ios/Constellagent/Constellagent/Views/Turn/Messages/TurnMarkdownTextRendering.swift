...
// Depends on: Foundation, SwiftUI, ConstellagentTextKit, TurnMessageCaches, TurnMessageRegexCache
...
// Wraps the default ConstellagentTextKit markdown parser with a bounded AttributedString
...
struct MarkdownTextView: View {
    ...
    var body: some View {
        ...
        // Keep prose on the app font, but let ConstellagentTextKit own markdown/code layout to avoid block sizing regressions.
        // ConstellagentTextKit intentionally keeps the `.textual` namespace for its SwiftUI modifiers.
...
private enum StreamingMarkdownBlockSplitter {
    ...
    // Keep the newest chunk intact so ConstellagentTextKit can apply native paragraph/list/code spacing
