...
// Depends on: Foundation, ImageIO, Runestone, SwiftUI, UIKit, ConstellagentService workspace preview APIs
...
struct AssistantMarkdownImagePreviewButton: View {
        ...
        HStack(spacing: 12) {
            ZStack {
                ...
                if isAutoLoadingPreview {
                    ...
                    ConstellagentIcon.image(systemName: "photo")
            ...
            ConstellagentIcon.image(systemName: "arrow.up.left.and.arrow.down.right")
    ...
    private var autoPreviewLoadKey: String {
        "\(reference.id)|\(constellagent.connectionPhase)"
    }

    private var canAutoLoadPreview: Bool {
        constellagent.connectionPhase == .connected
    }
...
struct WorkspaceLinkedFilePreviewScreen: View {
        ...
        ZStack(alignment: .top) {
            ...
            VStack(spacing: 16) {
                ...
                if isLoading || errorMessage == nil {
                    ...
                    ConstellagentIcon.image(systemName: "doc.text")
    ...
    private func loadSVGPayload() async throws -> WorkspaceSVGFilePreviewPayload {
        let result = try await constellagent.readWorkspaceImage(
            path: request.path,
            cwd: request.currentWorkingDirectory
        )
        guard let data = result.data,
              let source = String(data: data, encoding: .utf8) else {
            throw ConstellagentServiceError.invalidResponse("SVG preview response did not include readable SVG data.")
        }
    ...
    private func loadTextPayload() async throws -> WorkspaceTextFileReadResult {
        try await constellagent.readWorkspaceTextFile(
            path: request.path,
            cwd: request.currentWorkingDirectory
        )
    }
            ...
            .toolbar {
                ...
                ToolbarItemGroup(placement: .confirmationAction) {
                    Button {
                        ...
                        ConstellagentIcon.image(systemName: "arrow.clockwise")
                    }
                    ...
                    Button {
                        ...
                        ConstellagentIcon.image(systemName: isShowingCopiedConfirmation ? "checkmark" : "doc.on.doc")
                    }
...
private struct WorkspaceFileTypeIcon: View {
    ...
    var body: some View {
        ...
        ZStack {
            ...
            ConstellagentIcon.image(systemName: style.symbolName)
                .font(AppFont.system(size: 17, weight: .semibold))
...
actor WorkspaceImagePreviewCache {
    ...
    func preview(for result: WorkspaceImageReadResult) async throws -> CommandImagePreviewPayload {
        ...
        guard let data = result.data else {
            throw ConstellagentServiceError.invalidResponse("Cached image preview was unavailable.")
        }
    ...
    static func decode(_ data: Data) async throws -> CommandImagePreviewPayload {
        try await Task.detached(priority: .userInitiated) {
            ...
            guard let source = CGImageSourceCreateWithData(data as CFData, sourceOptions) else {
                throw ConstellagentServiceError.invalidResponse("The file is not a readable image.")
            }
