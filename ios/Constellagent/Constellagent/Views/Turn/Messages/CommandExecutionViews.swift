...
nonisolated struct AssistantMarkdownImageReference: Identifiable, Equatable {
    ...
    var isConstellagentGeneratedImage: Bool {
        AssistantMarkdownImageReferenceParser.isConstellagentGeneratedImagePath(path)
    }
}
...
nonisolated enum AssistantMarkdownImageReferenceParser {
    ...
    static func isConstellagentGeneratedImagePath(_ path: String) -> Bool {
        path.trimmingCharacters(in: .whitespacesAndNewlines)
            .replacingOccurrences(of: "\\", with: "/")
            .lowercased()
            .contains("/.constellagent/generated_images/")
    }
...
struct CommandExecutionDetailSheet: View {
    ...
    private var commandSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            ConstellagentIcon.label("Command", systemName: "terminal.fill")
    ...
    private var outputSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            Button {
                ...
                HStack(spacing: 6) {
                    ConstellagentIcon.image(systemName: isOutputExpanded ? "chevron.down" : "chevron.right")
