...
// Keep ConstellagentTextKit selection out of the scrolling timeline. This is shared by both
...
private struct MessageRowMessageSignature: Equatable {
    ...
    let textFingerprint: ConstellagentMessageTextRenderSignature
    ...
    let deliveryState: ConstellagentMessageDeliveryState
    let attachments: [MessageRowAttachmentSignature]
    let planState: MessageRowPlanStateSignature?
    let planPresentation: ConstellagentPlanPresentation?
    ...
    init(_ message: ConstellagentMessage) {
        self.id = message.id
...
private struct MessageRowAttachmentSignature: Equatable {
    let id: String
    let thumbnailFingerprint: ConstellagentTextContentFingerprint
    let payloadFingerprint: ConstellagentTextContentFingerprint?
    let sourceFingerprint: ConstellagentTextContentFingerprint?

    init(_ attachment: ConstellagentImageAttachment) {
...
private struct MessageRowProposedPlanSignature: Equatable {
    ...
    init(_ plan: ConstellagentProposedPlan) {
...
private struct MessageRowPlanStateSignature: Equatable {
    ...
    init(_ planState: ConstellagentPlanState) {
...
private struct MessageRowPlanStepSignature: Equatable {
    ...
    init(_ step: ConstellagentPlanStep) {
        self.id = step.id
...
private struct MessageRowSubagentActionSignature: Equatable {
    ...
    init(_ action: ConstellagentSubagentAction) {
...
private struct MessageRowSubagentRefSignature: Equatable {
    ...
    init(_ ref: ConstellagentSubagentRef) {
...
private struct MessageRowSubagentStateSignature: Equatable {
    ...
    init(_ state: ConstellagentSubagentState) {
...
private struct MessageRowStructuredInputRequestSignature: Equatable {
    ...
    init(_ request: ConstellagentStructuredUserInputRequest) {
...
private struct MessageRowStructuredInputQuestionSignature: Equatable {
    ...
    init(_ question: ConstellagentStructuredUserInputQuestion) {
        self.id = question.id
...
private struct MessageRowStructuredInputOptionSignature: Equatable {
    ...
    init(_ option: ConstellagentStructuredUserInputOption) {
        self.id = option.id
...
private struct TimelineShowMoreTextButton: View {
    ...
    var body: some View {
        Button {
            ...
            HStack(spacing: 6) {
                ConstellagentIcon.image(systemName: "chevron.down")
...
func timelineDisplayText(for message: ConstellagentMessage) -> String {
...
func timelineActionText(for message: ConstellagentMessage) -> String {
...
struct MessageRow: View, Equatable {
    ...
    var planSessionSource: ConstellagentPlanSessionSource? = nil
    var allowsAssistantPlanFallbackRecovery: Bool = false
    var assistantTurnCompleted: Bool = false
    var threadMessagesForPlanMatching: [ConstellagentMessage] = []
    ...
    // True while the sticky "Constellagent is thinking" row is visible at the bottom of the timeline.
    ...
    var assistantRevertAction: ((ConstellagentMessage) -> Void)? = nil
    var subagentOpenAction: ((ConstellagentSubagentThreadPresentation) -> Void)? = nil
    ...
    private func assistantView(text: String, actionText: String, renderModel: MessageRowRenderModel) -> some View {
        ...
        let assistantProposedPlanCandidate = shouldParseStructuredAssistantContent
            ...
            ? (message.proposedPlan ?? ConstellagentProposedPlanParser.parse(from: bodyText))
            : nil
        ...
        let proposedPlan = !isNativePlanSession
            ? (assistantProposedPlanCandidate
                ?? (
                    ...
                    ? ConstellagentProposedPlanParser.parseAssistantFallback(from: bodyText)
                            : nil
                ))
            : nil
        let renderedPlanText = assistantProposedPlanCandidate == nil
            ? bodyText
            : (
                ConstellagentProposedPlanParser.containsEnvelope(in: bodyText)
                    ? (ConstellagentProposedPlanParser.removingEnvelope(from: bodyText) ?? "")
                    : ""
            )
    ...
    private func selectableTextActions(text: String, usesMarkdownSelection: Bool) -> some View {
        if let selectableText = timelineSelectableActionText(text) {
            ...
            Button {
                ...
                ConstellagentIcon.menuLabel("Copy", systemName: "doc.on.doc")
            }
        }
    }
