...
// Depends on: Foundation, ConstellagentMessage, AssistantMarkdownImageReferenceParser, CodeCommentDirectiveParser
...
struct TurnTimelineToolBurstGroup: Identifiable, Equatable {
    ...
    init(messages: [ConstellagentMessage]) {
        self.messages = messages
        self.id = "tool-burst:\(messages.first?.id ?? "unknown")"
    }

    var pinnedMessages: [ConstellagentMessage] {
        Array(messages.prefix(Self.collapsedVisibleCount))
    }

    var overflowMessages: [ConstellagentMessage] {
...
struct TurnTimelinePreviousMessagesGroup: Identifiable, Equatable {
    ...
    init(finalMessage: ConstellagentMessage, messages: [ConstellagentMessage]) {
...
enum TurnTimelineRenderItem: Identifiable, Equatable {
    case message(ConstellagentMessage)
...
enum TurnTimelineRenderProjection {
    ...
    static func project(messages: [ConstellagentMessage], completedTurnIDs: Set<String> = []) -> [TurnTimelineRenderItem] {
        var items: [TurnTimelineRenderItem] = []
        var bufferedToolMessages: [ConstellagentMessage] = []
        ...
        let previousReplacementByIndex = finalCollapsePlan.reduce(into: [Int: ConstellagentMessage]()) { result, entry in
    ...
    private struct PreviousMessagesCollapse {
        ...
        let replacementFinalMessage: ConstellagentMessage?
    }

    private struct FileChangeCollapsePlan {
        let hiddenIndices: Set<Int>
        let replacementByIndex: [Int: ConstellagentMessage]
    }

    // Shows one end-of-turn file table even when the bridge streams multiple file-change snapshots.
    private static func fileChangeCollapsePlan(in messages: [ConstellagentMessage]) -> FileChangeCollapsePlan {
        ...
        var replacementByIndex: [Int: ConstellagentMessage] = [:]
    ...
    private static func mergeAdjacentFileChangeItems(
        ...
        var pendingFileChanges: [ConstellagentMessage] = []
    ...
    private static func shouldEagerlyCollapseFileChanges(_ messages: [ConstellagentMessage]) -> Bool {
        var totalBytes = 0
    ...
    private static func lastUserIndexBefore(_ index: Int, in messages: [ConstellagentMessage], turnID: String) -> Int? {
    ...
    private static func isPriorityVisibleMessage(_ message: ConstellagentMessage, finalMessage: ConstellagentMessage? = nil) -> Bool {
    ...
    private static func isReplayOfFinalAssistant(_ message: ConstellagentMessage, finalMessage: ConstellagentMessage) -> Bool {
        guard message.role == .assistant,
    ...
    private static func shouldPreserveReplayAsPreviousMessage(
        ...
        finalMessage: ConstellagentMessage
    ) -> Bool {
        if isCommentaryAssistantPhase(message.assistantPhase),
           isFinalAnswerAssistantPhase(finalMessage.assistantPhase) {
            return true
    ...
    private static func finalMessageReplacingCollapsedArtifacts(
        finalMessage: ConstellagentMessage,
        collapsedMessages: [ConstellagentMessage],
        generatedImageArtifacts: [ConstellagentMessage]
    ...
    private static func isGeneratedImageArtifactAlreadyInFinal(_ message: ConstellagentMessage, finalMessage: ConstellagentMessage) -> Bool {
        guard message.role == .assistant,
        ...
        guard !artifactPaths.isEmpty,
              artifactPaths.allSatisfy({ AssistantMarkdownImageReferenceParser.isConstellagentGeneratedImagePath($0) }) else {
            return false
        }
    ...
    private static func isGeneratedImageArtifactOnly(_ message: ConstellagentMessage) -> Bool {
        guard message.role == .assistant,
              !message.isStreaming,
        ...
        return !imageReferences.isEmpty
            && imageReferences.allSatisfy(\.isConstellagentGeneratedImage)
    }
    ...
    private static func isAssistantPriorityArtifactOnly(_ message: ConstellagentMessage) -> Bool {
    ...
    private static func isToolBurstCandidate(_ message: ConstellagentMessage) -> Bool {
    ...
    private static func shouldSkipVisualRow(_ message: ConstellagentMessage) -> Bool {
    ...
    private static func canShareToolBurst(previous: ConstellagentMessage, incoming: ConstellagentMessage) -> Bool {
