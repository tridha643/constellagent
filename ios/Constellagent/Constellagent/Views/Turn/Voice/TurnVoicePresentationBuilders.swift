...
enum TurnVoiceRecoveryPresentationBuilder {
    static func presentation(for reason: ConstellagentVoiceFailureReason) -> VoiceRecoveryPresentation {
        switch reason {
        case .reconnectRequired:
            return VoiceRecoveryPresentation(
                snapshot: snapshot(
                    summary: "Reconnect to your device to use voice mode.",
                    detail: "Keep the Constellagent bridge running on your paired device, then try the microphone again.",
                    status: .interrupted,
        ...
        case .bridgeSessionUnsupported:
            return VoiceRecoveryPresentation(
                snapshot: snapshot(
                    ...
                    detail: "Restart Constellagent on your device, then reconnect this iPhone. If it still happens, update Constellagent on your device and pair again.",
        ...
        case .chatGPTRequired:
            return setupHelpPresentation(
                ...
                detail: "Restart or update Constellagent on the paired device. Current voice mode can use ChatGPT or an OpenAI API key without sending the token to your phone."
            )
        case .microphonePermissionRequired:
            return VoiceRecoveryPresentation(
                snapshot: snapshot(
                    summary: "Microphone access is off for Constellagent.",
                    detail: "Open iPhone Settings, allow Microphone for Constellagent, then try recording again.",
                    status: .actionRequired,
        ...
        case .recorderUnavailable:
            return VoiceRecoveryPresentation(
                snapshot: snapshot(
                    summary: "Constellagent could not start the recorder.",
        ...
        case .generic(let message):
            return VoiceRecoveryPresentation(
                snapshot: ConnectionRecoverySnapshot(
                    title: "Voice Mode",
                    summary: message ?? "Voice mode unavailable.",
                    status: .actionRequired,
                    trailingStyle: .none
                ),
                action: .none
        ...
        default:
            return VoiceRecoveryPresentation(
                snapshot: ConnectionRecoverySnapshot(
                    title: "Voice Mode",
                    summary: "Voice mode unavailable.",
                    status: .actionRequired,
                    trailingStyle: .none
                ),
                action: .none
