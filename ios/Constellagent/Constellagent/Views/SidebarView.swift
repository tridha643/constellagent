...
// Layer: View
// Exports: SidebarView
// Depends on: ConstellagentService, SidebarHeaderView, SidebarThreadListView,
...
struct SidebarView<ConnectionEmptyStatePanel: View, ConnectionEmptyStateFooter: View>: View {
    ...
    @Binding var selectedThread: ConstellagentThread?
    @Binding var isSearchActive: Bool
    var showsInlineCloseButton: Bool = false
    var isVisible: Bool = true
    let connectionPhase: ConstellagentConnectionPhase
    ...
    let onOpenThread: (ConstellagentThread) -> Void
    ...
    @State private var cachedRunBadges: [String: ConstellagentThreadRunBadgeState] = [:]
    ...
    var body: some View {
        ...
        threadListWithBottomBar
            ...
            .task {
                debugSidebarLog("task start visible=\(isVisible) threadCount=\(constellagent.threads.count)")
                rebuildGroupedThreads()
                rebuildCachedSidebarState()
                await refreshProjectlessChatRoots()
                if constellagent.isConnected, constellagent.threads.isEmpty {
                    await refreshThreads()
                }
            }
            .onChange(of: constellagent.threads) { _, _ in
                debugSidebarLog(
                    "threads changed while \(isVisible ? "visible" : "hidden-prewarmed") "
                        + "threadCount=\(constellagent.threads.count)"
                )
                rebuildGroupedThreads()
                rebuildCachedSidebarState()
            }
            ...
            .onChange(of: constellagent.pinnedThreadIDs) { _, _ in
                debugSidebarLog("pinned threads changed count=\(constellagent.pinnedThreadIDs.count)")
                rebuildGroupedThreads()
            }
            .onChange(of: constellagent.isConnected) { _, isConnected in
            ...
            .overlay {
                if SidebarThreadsLoadingPresentation.shouldShowOverlay(
                    isLoadingThreads: constellagent.isLoadingThreads,
                    threadCount: constellagent.threads.count
                ) {
                    ProgressView()
                        .padding()
    ...
    private func refreshThreads() async {
        guard constellagent.isConnected else { return }
        let startedAt = Date()
        debugSidebarLog("refreshThreads start threadCount=\(constellagent.threads.count)")
        do {
            try await constellagent.listThreads()
            debugSidebarLog(
            ...
            // Error stored in ConstellagentService.
        }
    }

    private func handleForceRefreshTap() {
        guard constellagent.isConnected else { return }
        Task { @MainActor in
            await forceRefreshFromDesktop()
        }
    }

    private func forceRefreshFromDesktop() async {
        guard constellagent.isConnected else { return }
        let startedAt = Date()
        let activeThreadToRefresh = selectedThread?.id ?? constellagent.activeThreadId
        debugSidebarLog("forceRefresh start activeThread=\(activeThreadToRefresh ?? "none")")

        await refreshThreads()
        await refreshProjectlessChatRoots()
        await constellagent.refreshInactiveRunningBadgeThreads()
        if let activeThreadToRefresh {
            await constellagent.syncActiveThreadState(threadId: activeThreadToRefresh)
            await constellagent.syncThreadHistory(threadId: activeThreadToRefresh, force: true)
        }

        debugSidebarLog(
            "forceRefresh success durationMs=\(Int(Date().timeIntervalSince(startedAt) * 1000)) "
                + "threadCount=\(constellagent.threads.count)"
        )
    }
    ...
    private func refreshProjectlessChatRoots() async {
        ...
        do {
            let roots = try await constellagent.fetchProjectlessChatRoots().roots
    ...
    private func handleNewChatTap(preferredProjectPath: String?) {
        ...
        Task { @MainActor in
            ...
            do {
                ...
                guard let message = constellagent.userFacingTurnErrorMessageForFooter(from: error) else { return }
    ...
    private func selectThread(_ thread: ConstellagentThread) {
    ...
    private func archivePendingProjectGroup() {
        ...
        let threadIDs = SidebarThreadGrouping.liveThreadIDsForProjectGroup(group, in: constellagent.threads)
        let selectedThreadWasArchived = selectedThread.map { selected in
            threadIDs.contains(selected.id)
        } ?? false

        _ = constellagent.archiveThreadGroup(threadIDs: threadIDs)

        if selectedThreadWasArchived {
            selectedThread = constellagent.threads.first(where: { thread in
    ...
    private func deletePendingProjectGroupLocally() {
        ...
        let threadIDs = SidebarThreadGrouping.allThreadIDsForProjectGroup(group, in: constellagent.threads)
        let selectedThreadWasDeleted = selectedThread.map { selected in
            threadIDs.contains(selected.id)
        } ?? false

        _ = constellagent.deleteLocalThreadGroup(threadIDs: threadIDs)

        if selectedThreadWasDeleted {
            selectedThread = constellagent.threads.first { thread in
                thread.syncState == .live && !threadIDs.contains(thread.id)
            }
        }
    ...
    private func rebuildGroupedThreads() {
        ...
        let source: [ConstellagentThread]
        if query.isEmpty {
            source = constellagent.threads
        } else {
            source = constellagent.threads.filter {
        ...
        groupedThreads = SidebarThreadGrouping.makeGroups(
            from: source,
            pinnedThreadIDs: constellagent.pinnedThreadIDs,
    ...
    private func groupingFingerprint(query: String, source: [ConstellagentThread]) -> Int {
        var hasher = Hasher()
        hasher.combine(query)
        hasher.combine(selectedContentScope)
        hasher.combine(projectlessChatRootPaths)
        hasher.combine(constellagent.pinnedThreadIDs)
    ...
    private var badgeFingerprint: Int {
        var hasher = Hasher()
        for thread in constellagent.threads {
            hasher.combine(thread.id)
            if let badge = constellagent.threadRunBadgeState(for: thread.id) {
    ...
    private func rebuildCachedRunBadges() {
        ...
        var byThreadID: [String: ConstellagentThreadRunBadgeState] = [:]
        for thread in constellagent.threads {
            if let state = constellagent.threadRunBadgeState(for: thread.id) {
        ...
        debugSidebarLog(
            "rebuildCachedRunBadges durationMs=\(Int(Date().timeIntervalSince(startedAt) * 1000)) "
                + "threadCount=\(constellagent.threads.count) cached=\(cachedRunBadges.count)"
        )
    }
    ...
    private var scopedSidebarThreads: [ConstellagentThread] {
    ...
    private var canCreateThread: Bool {
        constellagent.isConnected && constellagent.isInitialized && !isCreatingThread
    }
    ...
    private var shouldShowConnectionEmptyState: Bool {
        guard !constellagent.isConnected else { return false }
        guard constellagent.threads.isEmpty else { return false }
    ...
    private var threadList: some View {
        SidebarThreadListView(
            ...
            showsTimestampRefreshIndicator: { constellagent.snapshotOnlyPinnedThreadIDs.contains($0.id) },
            ...
            onPinToggleThread: { thread in
                if constellagent.isThreadPinned(thread.id) {
                    constellagent.unpinThread(thread.id)
                } else {
                    constellagent.pinThread(thread.id)
                }
                rebuildGroupedThreads()
            },
            onArchiveToggleThread: { thread in
                if thread.syncState == .archivedLocal {
                    constellagent.unarchiveThread(thread.id)
                } else {
                    constellagent.archiveThread(thread.id)
                    if selectedThread?.id == thread.id {
                        selectedThread = nil
                    }
                }
    ...
    private var overflowMenuActions: SidebarOverflowMenuActions {
        SidebarOverflowMenuActions(
            ...
            onRefresh: handleForceRefreshTap,
    ...
    func body(content: Content) -> some View {
        content
            ...
            .alert(
                ...
                Text("Chats for this project will be deleted only from Constellagent on this phone. Nothing is removed from your device or Constellagent observer.")
            }
            .alert(
