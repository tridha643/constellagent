...
// Depends on: SwiftUI, UIKit, ConstellagentService, MyDevicesPresentation, ConstellagentIcon, UIKitMenuButton
...
struct MyDevicesSettingsSheet: View {
    ...
    private var devices: [MyDeviceRowModel] {
        ...
        return MyDevicesPresentation.rowModels(from: constellagent, switchingDeviceId: switchingDeviceId)
    }
        ...
        let switchActions: [UIMenuElement] = pickerDevices
            .map { candidate in
                UIAction(
                    ...
                    image: ConstellagentIcon.menuUIImage(systemName: MyDevicesPresentation.macIconSystemName),
    ...
    private var requiresSwitchConfirmation: Bool {
        !constellagent.runningThreadIDs.isEmpty
            || !constellagent.protectedRunningFallbackThreadIDs.isEmpty
            || !constellagent.activeTurnIdByThread.isEmpty
    }
