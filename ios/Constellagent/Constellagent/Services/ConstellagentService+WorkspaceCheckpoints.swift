// FILE: ConstellagentService+WorkspaceCheckpoints.swift
...
//   WorkspaceCheckpointRestoreApplyResult, ConstellagentService checkpoint helpers
...
extension ConstellagentService {
    ...
    private func checkpointResultObject(from response: RPCMessage) throws -> RPCObject {
        guard let result = response.result?.objectValue else {
            throw ConstellagentServiceError.invalidResponse("Workspace checkpoint response was missing a result.")
        }
        return result
    }
