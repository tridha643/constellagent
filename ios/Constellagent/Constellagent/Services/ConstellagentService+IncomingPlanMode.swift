// FILE: ConstellagentService+IncomingPlanMode.swift
// Purpose: Handles plan-mode notifications and structured user-input requests.
// Layer: Service
// Exports: ConstellagentService plan-mode incoming handlers
// Depends on: ConstellagentService+Incoming shared routing helpers
...
private extension ConstellagentService {
    ...
    func decodePlanSteps(from value: JSONValue?) -> [ConstellagentPlanStep] {
    ...
    func decodeStructuredUserInputQuestions(from value: JSONValue?) -> [ConstellagentStructuredUserInputQuestion] {
        let items = value?.arrayValue ?? []
        return items.compactMap { value in
            guard let object = value.objectValue,
            ...
            let options = (object["options"]?.arrayValue ?? []).compactMap { optionValue -> ConstellagentStructuredUserInputOption? in
                ...
                return ConstellagentStructuredUserInputOption(label: label, description: description)
            }

            return ConstellagentStructuredUserInputQuestion(
                id: id,
                header: header,
                question: question,
