// FILE: ConstellagentService+Sync.swift
// Purpose: Near-real-time sync loop and server-authoritative thread reconciliation.
// Layer: Service
// Exports: ConstellagentService sync APIs
// Depends on: ConstellagentThread, ConstellagentServiceError
...
extension ConstellagentService {
    ...
    func startSyncLoop() {
        ...
        // feel closer to live on iPhone even when Constellagent.app itself doesn't push updates.
    ...
    func reconcileLocalThreadsWithServer(_ serverThreads: [ConstellagentThread]) {
        ...
        var merged: [String: ConstellagentThread] = [:]
    ...
    func handleMissingThread(_ threadId: String) {
        ...
        if let index = threadIndex(for: threadId) {
            ...
            threads.append(ConstellagentThread(id: threadId, title: ConstellagentThread.defaultDisplayTitle, syncState: .archivedLocal))
    ...
    func applyAutomaticThreadTitle(
        ...
        let currentCandidates = [persistedName, currentName, currentTitle, currentDisplayTitle]
            ...
            .filter { !$0.isEmpty }
        let defaultTitle = Self.normalizedAutomaticTitleComparisonValue(ConstellagentThread.defaultDisplayTitle)
    ...
    func debugSyncLog(_ message: String) {
        ...
        print("[ConstellagentSync] \(message)")
#endif
    }
    ...
    func updateBackgroundRunGraceTask() {
        ...
        let taskID = UIApplication.shared.beginBackgroundTask(withName: "ConstellagentRunGrace") { [weak self, taskBox] in
            ...
            // we hop back into ConstellagentService's MainActor-isolated state.
