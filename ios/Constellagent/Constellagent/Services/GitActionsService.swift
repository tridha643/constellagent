...
// Depends on: ConstellagentService, GitActionModels
...
enum GitActionsError: LocalizedError {
    ...
    private func userMessage(for code: String?, fallback: String?) -> String {
        ...
        case "missing_local_repo": return "Run `constellagent up` from an existing local directory first."
...
final class GitActionsService {
    ...
    init(constellagent: ConstellagentService, workingDirectory: String?) {
    ...
    // Creates a Constellagent-managed detached worktree rooted under CODEX_HOME/worktrees.
    ...
    func runStackedAction(
        ...
        if let onProgress {
            ...
            constellagent.registerGitStackedActionProgressHandler(progressId: id, handler: onProgress)
            progressId = id
        } else {
            progressId = nil
        }
        ...
        defer {
            if let progressId {
                constellagent.unregisterGitStackedActionProgressHandler(progressId: progressId)
            }
        }
    ...
    private func request(method: String, params: [String: JSONValue] = [:]) async throws -> [String: JSONValue] {
        ...
        do {
            let response = try await constellagent.sendRequest(method: method, params: rpcParams)
    ...
    private func rememberRepoRoot(from result: GitRepoSyncResult?) {
        constellagent.rememberRepoRoot(result?.repoRoot, forWorkingDirectory: workingDirectory)
    }
}
