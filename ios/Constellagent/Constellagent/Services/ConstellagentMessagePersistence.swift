// FILE: ConstellagentMessagePersistence.swift
// Purpose: Persists per-thread message timelines to disk between app launches.
// Layer: Service
// Exports: ConstellagentMessagePersistence
// Depends on: Foundation, CryptoKit, ConstellagentMessage

import CryptoKit
import Foundation

nonisolated struct ConstellagentMessagePersistence {
    // v6 encrypts the on-device message cache while keeping backward-compatible legacy fallbacks.
    private let fileName = "constellagent-message-history-v6.bin"
    private let legacyFileNames = [
        "constellagent-message-history-v5.json",
        "constellagent-message-history-v4.json",
        "constellagent-message-history-v3.json",
        "constellagent-message-history-v2.json",
        "constellagent-message-history.json",
    ]

    // Loads the saved message map from disk. Returns an empty store on failure.
    func load(macDeviceId: String? = nil, includeLegacyFallback: Bool = false) -> [String: [ConstellagentMessage]] {
        ...
        for fileURL in storeURLs(macDeviceId: macDeviceId, includeLegacyFallback: includeLegacyFallback) {
            ...
            if fileURL.lastPathComponent == fileName,
               ...
               let value = try? decoder.decode([String: [ConstellagentMessage]].self, from: decrypted) {
                return sanitizedForPersistence(value)
            }

            if let value = try? decoder.decode([String: [ConstellagentMessage]].self, from: data) {
                return sanitizedForPersistence(value)
            }
        }
    ...
    func save(_ value: [String: [ConstellagentMessage]], macDeviceId: String? = nil) {
    ...
    private func storeURLs(macDeviceId: String?, includeLegacyFallback: Bool = false) -> [URL] {
        ...
        let bundleID = Bundle.main.bundleIdentifier ?? "com.constellagentmobile.app"
    ...
    private func messageHistoryKey() -> SymmetricKey {
        if let storedKey = SecureStore.readData(for: ConstellagentSecureKeys.messageHistoryKey) {
            return SymmetricKey(data: storedKey)
        }

        let newKey = SymmetricKey(size: .bits256)
        let keyData = newKey.withUnsafeBytes { Data($0) }
        SecureStore.writeData(keyData, for: ConstellagentSecureKeys.messageHistoryKey)
        return newKey
    }
    ...
    private func sanitizedForPersistence(_ value: [String: [ConstellagentMessage]]) -> [String: [ConstellagentMessage]] {
