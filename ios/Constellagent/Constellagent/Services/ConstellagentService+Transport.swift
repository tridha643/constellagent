// FILE: ConstellagentService+Transport.swift
// Purpose: Outbound JSON-RPC transport and pending-response coordination.
// Layer: Service
// Exports: ConstellagentService transport internals
// Depends on: Network.NWConnection, URLSessionWebSocketTask, CryptoKit, Security, ConstellagentURLSessionWebSocketDelegate
...
let constellagentWebSocketMaximumMessageSizeBytes = 16 * 1024 * 1024

private enum ConstellagentWebSocketKeepAlivePolicy {
...
private final class ConstellagentForegroundProbeWaiter: @unchecked Sendable {
...
private enum ConstellagentRelayTransportPreference {
    case manualTCP
...
private struct ConstellagentConnectionReadyWaitConfiguration: Sendable {
    let logLabel: String
    let timeoutNanoseconds: UInt64
    let timeoutMessage: String
}

private final class ConstellagentConnectionReadyWaitState: @unchecked Sendable {
    private let lock = NSLock()
...
private struct ConstellagentManualWebSocketEndpoint {
    let host: String
    let port: NWEndpoint.Port
    let scheme: String
}

nonisolated private func constellagentLogPairingTransport(_ message: String) {
...
extension ConstellagentService {
    ...
    func validateOutgoingWebSocketMessageSize(_ text: String) throws {
        ...
        guard payloadSize <= constellagentWebSocketMaximumMessageSizeBytes else {
    ...
    func sendRequest(
        ...
        // Constellagent bridge speaks `session.*` / `git.*` / `workspace.*`
        // instead of the upstream `thread/*` / `turn/*` etc. Translate at the
        // single chokepoint; per-extension code keeps its remodex shape.
        let wireMethod = ConstellagentBridgeMethodMapping.map(method: method)
        let workspacePathHint = thread(for: activeThreadId ?? "")?.gitWorkingDirectory
        let wireParams = ConstellagentBridgeMethodMapping.adaptParams(
            codexMethod: method,
            params: params,
        ...
        if ConstellagentBridgeMethodMapping.isStubbed(method: method) {
            // No bridge equivalent — synthesise an empty response locally so
            // the UI keeps moving. See ConstellagentService+ProtocolMapping.
            return ConstellagentBridgeMethodMapping.stubResponse(for: method, requestID: requestID)
        }
        ...
        pendingRequestDebugMeta[requestKey] = (method: method, startedAt: Date())
        if method == "turn/start" {
            debugMobileLog(
                "rpc send turn/start thread=\(params?.objectValue?["threadId"]?.stringValue ?? "") inputItems=\(params?.objectValue?["input"]?.arrayValue?.count ?? 0)"
            )
        }
        ...
        return try await withTaskCancellationHandler {
                ...
                Task {
                    do {
                        ...
                        if let pendingContinuation = pendingRequests.removeValue(forKey: requestKey) {
                            pendingRequestDebugMeta.removeValue(forKey: requestKey)
            ...
            Task { @MainActor [weak self] in
                ...
                self.pendingRequestDebugMeta.removeValue(forKey: requestKey)
    ...
    func scheduleRequestTimeoutIfNeeded(
        ...
        Task { @MainActor [weak self] in
            ...
            if let debugMeta = self.pendingRequestDebugMeta.removeValue(forKey: requestKey) {
                let durationMs = Int(Date().timeIntervalSince(debugMeta.startedAt) * 1000)
                self.debugMobileLog(
                    "rpc timeout \(debugMeta.method) durationMs=\(durationMs) msg=\(timeoutMessage ?? method)"
                )
            }
            let message = timeoutMessage ?? "\(method) timed out while loading this chat."
            continuation.resume(
                throwing: ConstellagentServiceError.invalidInput(message)
            )
        }
    }
    ...
    func sendMessage(_ message: RPCMessage) async throws {
        ...
        guard let plaintext = String(data: payload, encoding: .utf8) else {
            throw ConstellagentServiceError.invalidResponse("Unable to encode outgoing JSON-RPC payload")
        }
    ...
    func sendRawText(_ text: String) async throws {
        ...
        let context = NWConnection.ContentContext(identifier: "constellagent-jsonrpc", metadata: [metadata])
        ...
        webSocketKeepAliveTask = Task { @MainActor [weak self] in
            while let self, !Task.isCancelled {
                let defaultInterval = self.isConstrainedNetwork
                    ? ConstellagentWebSocketKeepAlivePolicy.constrainedIntervalNanoseconds
                    : ConstellagentWebSocketKeepAlivePolicy.intervalNanoseconds
    ...
    func probeForegroundConnectionIfNeeded() async {
        ...
        let result = await foregroundConnectionProbeResult(
            timeoutNanoseconds: webSocketForegroundProbeTimeoutOverrideNanoseconds
                ?? ConstellagentWebSocketKeepAlivePolicy.foregroundProbeTimeoutNanoseconds
        )
    ...
    private func foregroundConnectionProbeResult(timeoutNanoseconds: UInt64) async -> Result<Void, Error> {
        await withCheckedContinuation { continuation in
            let waiter = ConstellagentForegroundProbeWaiter(continuation: continuation)
    ...
    func sendWebSocketKeepAlivePing() async throws {
        ...
        let context = NWConnection.ContentContext(identifier: "constellagent-keepalive-ping", metadata: [metadata])
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
            connection.send(
                content: Data(),
                contentContext: context,
                ...
                if let metadata = context?.protocolMetadata(definition: NWProtocolWebSocket.definition) as? NWProtocolWebSocket.Metadata,
                   metadata.opcode == .close {
                    self.handleReceiveError(
                        ConstellagentServiceError.disconnected,
        ...
        receiveRaw(on: connection) { [weak self] result in
            ...
            Task { @MainActor [weak self] in
                ...
                case .failure(let error):
                    self.handleReceiveError(error)
                case .success(nil):
                    self.handleReceiveError(ConstellagentServiceError.disconnected)
    ...
    func establishWebSocketConnection(url: URL, token: String, role: String? = nil) async throws -> ConstellagentWebSocketTransport {
        guard let scheme = url.scheme?.lowercased(),
              scheme == "ws" || scheme == "wss" else {
            throw ConstellagentServiceError.invalidServerURL(url.absoluteString)
        }

        let preference = relayTransportPreference(for: url)
        constellagentLogPairingTransport("using \(preference.logLabel) for \(url.host ?? "unknown-host")")
    ...
    func establishManualTCPWebSocketConnection(
        ...
        token: String,
        role: String? = nil
    ) async throws -> ConstellagentWebSocketTransport {
        ...
        let waitConfiguration = ConstellagentConnectionReadyWaitConfiguration(
        ...
        constellagentLogPairingTransport("opening manual TCP websocket")
        ...
        do {
            ...
            constellagentLogPairingTransport("manual TCP websocket connected")
        } catch {
            connection.cancel()
            throw error
        }
    ...
    func establishNWWebSocketConnection(
        ...
        webSocketOptions.maximumMessageSize = constellagentWebSocketMaximumMessageSizeBytes
        ...
        constellagentLogPairingTransport("opening NWConnection websocket")
    ...
    func establishURLSessionWebSocketConnection(
        ...
        let delegate = ConstellagentURLSessionWebSocketDelegate()
        let session = URLSession(configuration: configuration, delegate: delegate, delegateQueue: nil)
        let task = session.webSocketTask(with: request)
        task.maximumMessageSize = constellagentWebSocketMaximumMessageSizeBytes
        let connectionTimeoutNanoseconds: UInt64 = 12_000_000_000

        constellagentLogPairingTransport("opening URLSessionWebSocketTask")
        task.resume()
        ...
        let timeoutTask = Task { [weak task, weak delegate] in
            ...
            delegate?.resolveOpen(
                with: .failure(ConstellagentServiceError.invalidInput("Connection timed out after 12s"))
            )
        }
        ...
        do {
            ...
            constellagentLogPairingTransport("URLSessionWebSocketTask connected")
        } catch {
            constellagentLogPairingTransport("URLSessionWebSocketTask failed: \(urlSessionWebSocketDebugDescription(for: error))")
            task.cancel(with: .goingAway, reason: nil)
    ...
    private func waitUntilManualConnectionReady(
        ...
        configuration: ConstellagentConnectionReadyWaitConfiguration
    ) async throws {
    ...
    private func waitUntilConnectionReady(
        ...
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
            let waitState = ConstellagentConnectionReadyWaitState(
            ...
            connection.stateUpdateHandler = { state in
                ...
                constellagentLogPairingTransport("\(configuration.logLabel) state: \(state)")
                switch state {
                case .ready:
                ...
                case .failed(let error):
                    constellagentLogPairingTransport("\(configuration.logLabel) failed: \(error)")
                    waitState.finish(.failure(error))
                case .cancelled:
                    waitState.finish(.failure(ConstellagentServiceError.disconnected))
                default:
                    break
                }
            }
            ...
            let timeoutTask = Task { [weak connection, waitState] in
                ...
                guard !Task.isCancelled else { return }
                let timeoutError = ConstellagentServiceError.invalidInput(configuration.timeoutMessage)
                constellagentLogPairingTransport(waitState.timeoutLogMessage(label: configuration.logLabel))
    ...
    func performManualWebSocketHandshake(
        ...
        constellagentLogPairingTransport("sending manual TCP websocket upgrade request")
        ...
        while true {
            if let range = headerBytes.range(of: Data("\r\n\r\n".utf8)) {
                ...
                constellagentLogPairingTransport("manual TCP websocket upgrade accepted")
                return
            }
            ...
            if headerBytes.count > 65_536 {
                throw ConstellagentServiceError.invalidInput("Relay handshake response was too large")
            }
        }
    }

    private func relayTransportPreference(for url: URL) -> ConstellagentRelayTransportPreference {
        prefersDirectRelayTransport(for: url) ? .manualTCP : .networkWebSocket
    }

    private func manualWebSocketEndpoint(from url: URL) throws -> ConstellagentManualWebSocketEndpoint {
        ...
        return ConstellagentManualWebSocketEndpoint(host: host, port: port, scheme: scheme)
    }
    ...
    func validateManualWebSocketHandshakeResponse(headerData: Data, key: String) throws {
        guard let headerText = String(data: headerData, encoding: .utf8) else {
            throw ConstellagentServiceError.invalidInput("Relay handshake response could not be decoded")
        }

        let lines = headerText.components(separatedBy: "\r\n")
        guard let status = lines.first, status.contains(" 101 ") else {
            throw ConstellagentServiceError.invalidInput("Relay rejected websocket upgrade")
        }
        ...
        guard headers["sec-websocket-accept"] == expectedAccept else {
            throw ConstellagentServiceError.invalidInput("Relay returned an invalid websocket accept key")
        }
    }
