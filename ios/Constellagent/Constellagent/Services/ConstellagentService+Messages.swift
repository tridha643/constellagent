// FILE: ConstellagentService+Messages.swift
// Purpose: Owns per-thread message timelines, streaming merge logic, and persistence.
// Layer: Service
// Exports: ConstellagentService message APIs
...
private extension Array where Element == ConstellagentMessage {
...
extension ConstellagentService {
    ...
    func messages(for threadId: String) -> [ConstellagentMessage] {
    ...
    func shouldShowImmediateEmptyPlaceholder(
        ...
        return thread.displayTitle == ConstellagentThread.defaultDisplayTitle
    }
    ...
    func latestTurnTerminalState(for threadId: String) -> ConstellagentTurnTerminalState? {
        latestTurnTerminalStateByThread[threadId]
    }

    // Returns the terminal outcome for a specific turn when known.
    func turnTerminalState(for turnId: String?) -> ConstellagentTurnTerminalState? {
    ...
    func threadRunBadgeState(for threadId: String) -> ConstellagentThreadRunBadgeState? {
    ...
    func applyRunCompletionBadgeState(threadId: String, result: ConstellagentRunCompletionResult) {
        switch result {
        case .completed:
    ...
    func recordTurnTerminalState(
        threadId: String,
        turnId: String?,
        state: ConstellagentTurnTerminalState
    ) {
    ...
    func shouldTreatAsEmptyUnmaterializedThreadHistory(
        _ error: ConstellagentServiceError,
        threadId: String,
    ...
    // The first turn can be running before Constellagent has persisted a readable
    ...
    func watchRunningThreadIfNeeded(_ threadId: String?, ttl: TimeInterval = 30) {
        ...
        runningThreadWatchByID[threadId] = ConstellagentRunningThreadWatch(
            threadId: threadId,
            ...
            if supportsTurnPagination {
                do {
                    ...
                    if consumeUnsupportedTurnPagination(error, attemptedMethod: "thread/turns/list") {
                        ...
                        } catch let legacyError as ConstellagentServiceError {
                        ...
                        if let threadData = try? JSONEncoder().encode(JSONValue.object(threadObject)),
                           let decoded = try? JSONDecoder().decode(ConstellagentThread.self, from: threadData) {
    ...
    func appendUserMessage(
        ...
        if let lastMatchingUser = messagesByThread[threadId]?.last(where: { candidate in
            candidate.role == .user
                && Self.normalizedMessageText(candidate.text) == trimmedText
        }) {
            if lastMatchingUser.deliveryState == .pending {
                debugMobileLog(
                    "appendUser dedupe pending thread=\(threadId) msg=\(lastMatchingUser.id)"
                )
                return lastMatchingUser.id
            }
            // Retry/continuation paths can re-append the same outgoing text seconds
            // after the optimistic row was already confirmed for turn/start.
            if turnId == nil,
               lastMatchingUser.turnId == nil,
               Date().timeIntervalSince(lastMatchingUser.createdAt) < 30 {
                debugMobileLog(
                    "appendUser dedupe recent thread=\(threadId) msg=\(lastMatchingUser.id) ageSec=\(Int(Date().timeIntervalSince(lastMatchingUser.createdAt)))"
                )
                return lastMatchingUser.id
            }
        }

        let message = ConstellagentMessage(
            threadId: threadId,
            role: .user,
            text: trimmedText,
    ...
    func upsertPlanMessage(
        ...
        steps: [ConstellagentPlanStep]? = nil,
        isStreaming: Bool,
        ...
        var planState = messagesByThread[threadId]?[messageIndex].planState ?? ConstellagentPlanState()
        if let explanation {
    ...
    func upsertSubagentActionMessage(
        ...
        turnId: String?,
        itemId: String?,
        action: ConstellagentSubagentAction,
        isStreaming: Bool
    ) {
    ...
    func upsertStructuredUserInputPrompt(
        ...
        turnId: String?,
        itemId: String,
        request: ConstellagentStructuredUserInputRequest
    ) {
    ...
    func toolActivityTargetIndex(in messages: [ConstellagentMessage], turnId: String?) -> Int? {
    ...
    private func canReuseLiveToolActivityRow(_ candidate: ConstellagentMessage, incomingItemId: String) -> Bool {
    ...
    private func pruneDuplicateSystemRows(
        in threadMessages: inout [ConstellagentMessage],
        keepIndex: Int,
        kind: ConstellagentMessageKind,
        turnId: String,
    ...
    private func applyAssistantDeltaBatch(
        ...
        let deltaSequence = (assistantDeltaDebugSequenceByThread[threadId] ?? 0) + 1
        assistantDeltaDebugSequenceByThread[threadId] = deltaSequence
        if ConstellagentService.shouldSampleMobileDelta(deltaSequence),
           nextText != currentText {
            debugMobileLog(
                "assistant delta thread=\(threadId) turn=\(turnId) seq=\(deltaSequence) incomingChars=\(delta.count) totalChars=\(nextText.count)"
            )
        }
           ...
           let messageIndex = findMessageIndex(threadId: threadId, messageId: messageID) {
            ...
            if let explicitItemId,
               let existingItemIndex = messagesByThread[threadId]?.lastIndex(where: { candidate in
                ...
                let newMessage = ConstellagentMessage(
    ...
    private func generatedImageMergeTarget(
        in threadMessages: [ConstellagentMessage],
        turnId: String?,
        itemId: String?,
        imagePath: String,
        markdown: String
    ...
    private static func assistantCompletionTextPreservingImages(
        ...
        let existingImageReferences = AssistantMarkdownImageReferenceParser.references(in: existingText)
            .filter(\.isConstellagentGeneratedImage)
    ...
    func markMessageDeliveryState(
        threadId: String,
        messageId: String,
        state: ConstellagentMessageDeliveryState,
        turnId: String? = nil
    ) {
    ...
    func markTurnCompleted(threadId: String, turnId: String?) {
        ...
        if var threadMessages = messagesByThread[threadId] {
            var didMutate = false
            let belongsToCompletedTurn: (ConstellagentMessage) -> Bool = { message in
                if let resolvedTurnId {
            ...
            for index in threadMessages.indices where threadMessages[index].role == .system
                ...
                case .resultCompletedItem:
                    let nextPresentation: ConstellagentPlanPresentation = shouldFinalizePlanSteps ? .resultReady : .resultClosed
            ...
            if shouldFinalizePlanSteps {
                ...
                for index in threadMessages.indices where threadMessages[index].role == .system
                    ...
                    threadMessages[index].planState = ConstellagentPlanState(
...
extension ConstellagentService {
    ...
    func presentThreadCompletionBannerIfNeeded(threadId: String) {
        ...
        threadCompletionBanner = ConstellagentThreadCompletionBanner(
            threadId: threadId,
    ...
    func snapshotProjectionSourceMessages(
        threadId: String,
        from messages: [ConstellagentMessage],
    ...
    func visibleTailPreservingTurnArtifacts(
        visibleTail: [ConstellagentMessage],
        omittedPrefix: ArraySlice<ConstellagentMessage>
    ) -> [ConstellagentMessage] {
        ...
        let preservedArtifactKinds: Set<ConstellagentMessageKind> = [.fileChange, .plan]
    ...
    func isPreservableTurnArtifact(
        _ message: ConstellagentMessage,
        kinds: Set<ConstellagentMessageKind>
    ) -> Bool {
    ...
    func rebuildStoppedTurnIDs(for threadId: String, messages: [ConstellagentMessage]) -> Set<String> {
    ...
    func mergeHistoryTurnTerminalStates(
        threadId: String,
        terminalStatesByTurnID historyStates: [String: ConstellagentTurnTerminalState]
    ) -> Bool {
    ...
    func buildRepoRefreshSignal(for messages: [ConstellagentMessage]) -> String? {
    ...
    func assistantRevertStates(
        for threadId: String,
        projectedMessages: [ConstellagentMessage],
        workingDirectory: String?,
        messageRevision: Int,
    ...
    func triggerRunCompletionHapticIfNeeded(
        threadId: String,
        state: ConstellagentTurnTerminalState,
        previousState: ConstellagentTurnTerminalState?
    ) {
    ...
    func thinkingActivityTargetIndex(in messages: [ConstellagentMessage], turnId: String?) -> Int? {
    ...
    func appendMessage(_ message: ConstellagentMessage) {
    ...
    private func refreshDerivedPlanMetadata(in messages: inout [ConstellagentMessage], index: Int) {
    ...
    private func derivedProposedPlan(for message: ConstellagentMessage) -> ConstellagentProposedPlan? {
        ...
        if message.role == .system && message.kind == .plan {
            ...
            return ConstellagentProposedPlanParser.parsePlanItem(from: message.text)
        }

        return ConstellagentProposedPlanParser.parse(from: message.text)
    }
    ...
    private func resolvedPlanPresentation(
        requested: ConstellagentPlanPresentation,
        turnId: String?
    ...
    func syntheticStreamingItemId(turnId: String, kind: ConstellagentMessageKind) -> String {
        "turn:\(turnId)|kind:\(kind.rawValue)"
    }

    func syntheticSubagentActionItemIdPrefix(turnId: String) -> String {
        "turn:\(turnId)|kind:\(ConstellagentMessageKind.subagentAction.rawValue)|action:"
    }

    func streamingPlaceholderText(for kind: ConstellagentMessageKind) -> String {
        switch kind {
        case .thinking:
            return ""
        case .toolActivity:
    ...
    func isStreamingPlaceholder(_ text: String, for kind: ConstellagentMessageKind) -> Bool {
        guard text.utf8.count <= MessageTextProcessingPolicy.largeTextByteLimit else {
            return false
        }
        return Self.normalizedMessageText(text)
    ...
    func shouldPruneThinkingRowAfterTurnCompletion(_ message: ConstellagentMessage) -> Bool {
        ...
        if maxOverlap > 0 {
            for overlap in stride(from: maxOverlap, through: 1, by: -1) {
                if existingText.suffix(overlap) == incomingDelta.prefix(overlap) {
                    return appendAssistantDeltaFragment(
                        to: existingText,
                        incoming: String(incomingDelta.dropFirst(overlap))
                    )
                }
            }
        }
