// FILE: ConstellagentService+TrustedPairPresentation.swift
// Purpose: Derives a compact UI-facing summary for the connected or remembered host pair.
// Layer: Service extension
// Exports: ConstellagentTrustedPairPresentation, ConstellagentService trusted-pair presentation helpers
// Depends on: Foundation

import Foundation

struct ConstellagentTrustedPairPresentation: Equatable, Sendable {
    let deviceId: String?
    let title: String
    let name: String
...
enum SidebarComputerNicknameStore {
    private static let keyPrefix = "constellagent.sidebarComputerNickname."
...
extension ConstellagentService {
    ...
    var trustedPairPresentation: ConstellagentTrustedPairPresentation? {
        ...
        return ConstellagentTrustedPairPresentation(
...
private extension ConstellagentService {
    ...
    var visibleTrustedMacRecord: ConstellagentTrustedMacRecord? {
    ...
    var trustedPairFingerprint: String? {
        nonEmptyTrimmedString(secureMacFingerprint)
            ?? normalizedRelayMacIdentityPublicKey.map { constellagentSecureFingerprint(for: $0) }
            ?? visibleTrustedMacRecord.map { constellagentSecureFingerprint(for: $0.macIdentityPublicKey) }
    }
