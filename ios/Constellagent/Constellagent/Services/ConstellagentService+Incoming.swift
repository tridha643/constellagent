// FILE: ConstellagentService+Incoming.swift
// Purpose: Inbound message decoding and event routing.
// Layer: Service
// Exports: ConstellagentService inbound handlers
...
extension ConstellagentService {
    ...
    func handleIncomingRPCMessage(_ message: RPCMessage) {
        ...
        let debugMeta = pendingRequestDebugMeta.removeValue(forKey: requestKey)
        if let debugMeta {
            let durationMs = Int(Date().timeIntervalSince(debugMeta.startedAt) * 1000)
            if debugMeta.method == "turn/start" {
                debugMobileLog(
                    "rpc recv turn/start durationMs=\(durationMs) ok=\(message.error == nil) turn=\(extractTurnID(from: message.result) ?? "")"
                )
            } else if ConstellagentService.isMobileDebugLoggingEnabled {
                debugRuntimeLog(
                    "rpc recv \(debugMeta.method) durationMs=\(durationMs) ok=\(message.error == nil)"
                )
            }
        }
        ...
        if let rpcError = message.error {
            continuation.resume(throwing: ConstellagentServiceError.rpcError(rpcError))
        } else {
    ...
    func handleServerRequest(method: String, requestID: JSONValue, params: JSONValue?) {
        ...
        if method == "item/commandExecution/requestApproval"
            ...
            let paramsObject = params?.objectValue
            let request = ConstellagentApprovalRequest(
            ...
            let isDesktopMirroredApproval = paramsObject?["constellagentActionSource"]?.stringValue == desktopIpcActionSource
    ...
    func handleNotification(method: String, params: JSONValue?) {
        ...
        case "item/agentMessage/delta",
             "constellagent/event/agent_message_content_delta",
             "constellagent/event/agent_message_delta":
            appendAgentDelta(from: paramsObject)

        case "constellagent/event/user_message":
        ...
        case "constellagent/event/exec_command_begin",
             "constellagent/event/exec_command_output_delta",
             "constellagent/event/exec_command_end",
             "constellagent/event/background_event",
             "constellagent/event/read",
             "constellagent/event/search",
             "constellagent/event/list_files",
             "constellagent/event/image_generation_end":
            if handleLegacyConstellagentNamedEvent(method: method, paramsObject: paramsObject) {
                return
            }

        case "turn/diff/updated", "constellagent/event/turn_diff_updated", "constellagent/event/turn_diff":
            handleTurnDiffUpdated(paramsObject)

        case "constellagent/event/patch_apply_begin", "constellagent/event/patch_apply_end":
            if handleLegacyPatchApplyMethod(method: method, paramsObject: paramsObject) {
                return
            }

        case "constellagent/event":
            if handleLegacyConstellagentEnvelopeEvent(paramsObject) {
                return
            }

        case "image_generation_end":
            if handleLegacyConstellagentEventType(
        ...
        case "item/completed", "constellagent/event/item_completed", "constellagent/event/agent_message":
            appendCompletedAgentText(from: paramsObject)

        case "item/started", "constellagent/event/item_started":
            handleItemStarted(paramsObject)

        case "error", "constellagent/event/error", "turn/failed":
        ...
        case "terminal/event":
            break // terminal support is deferred — see ConstellagentDeferredStubs.swift

        default:
            if method.hasPrefix("constellagent/event/"),
               handleLegacyConstellagentNamedEvent(method: method, paramsObject: paramsObject) {
                return
            }
    ...
    private func noteDesktopMirroredActivityIfNeeded(
        ...
        let isDesktopMirroredEvent = paramsObject?["constellagentDesktopMirror"]?.boolValue == true
            || paramsObject?["constellagentRolloutLiveMirror"]?.boolValue == true
        let isMirrorActivityMethod = method.hasPrefix("item/")
            || method.hasPrefix("constellagent/event")
    ...
    private func handleThreadStarted(_ paramsObject: IncomingParamsObject?) {
        guard let paramsObject,
              ...
              let thread = decodeModel(ConstellagentThread.self, from: threadValue) else {
            return
        }
    ...
    private func handleTurnStarted(_ paramsObject: IncomingParamsObject?) {
        ...
        let isDesktopMirroredTurn = paramsObject?["constellagentDesktopMirror"]?.boolValue == true
            || paramsObject?["constellagentRolloutLiveMirror"]?.boolValue == true
    ...
    ) -> ConstellagentTurnTerminalState {
    ...
    private func runCompletionResult(for state: ConstellagentTurnTerminalState) -> ConstellagentRunCompletionResult? {
        switch state {
        case .completed:
            .completed
        case .failed:
    ...
    private func existingCommandExecutionRow(threadId: String, itemId: String) -> ConstellagentMessage? {
    ...
    // Supports legacy constellagent/event envelopes where `msg.type == "turn_diff"` and payload uses unified_diff.
    private func handleLegacyConstellagentEnvelopeEvent(_ paramsObject: IncomingParamsObject?) -> Bool {
        guard let paramsObject,
        ...
        return handleLegacyConstellagentEventType(
            eventType: eventType,
            payload: msgObject,
            paramsObject: paramsObject
        )
    ...
    private func handleLegacyConstellagentNamedEvent(
        method: String,
        paramsObject: IncomingParamsObject?
    ) -> Bool {
        guard method.hasPrefix("constellagent/event/"),
              let paramsObject else {
            return false
        }

        let eventType = method
            .replacingOccurrences(of: "constellagent/event/", with: "")
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .lowercased()
    ...
    private func handleLegacyConstellagentEventType(
        eventType: String,
    ...
    // Accepts legacy Constellagent token_count events, even when the runtime omits thread ids.
    ...
    private func appendEssentialActivityLine(threadId: String, turnId: String?, line: String) {
        ...
        recentActivityLineByThread[dedupeKey] = ConstellagentRecentActivityLine(line: trimmedLine, timestamp: now)
