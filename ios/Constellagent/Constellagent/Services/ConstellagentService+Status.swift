// FILE: ConstellagentService+Status.swift
// Purpose: Fetches and tracks status-sheet data such as ChatGPT rate limits.
// Layer: Service
// Exports: ConstellagentService status helpers
// Depends on: ConstellagentRateLimitStatus, RPCMessage
...
extension ConstellagentService {
    ...
    func refreshContextWindowUsage(threadId: String) async {
        ...
        do {
            ...
            guard let resultObject = response.result?.objectValue else {
                throw ConstellagentServiceError.invalidResponse("thread/contextWindow/read response missing payload")
            }
    ...
    func refreshRateLimits() async {
        ...
        do {
            ...
            guard let resultObject = response.result?.objectValue else {
                throw ConstellagentServiceError.invalidResponse("account/rateLimits/read response missing payload")
            }
...
private extension ConstellagentService {
    ...
    func decodeRateLimitBuckets(from payloadObject: IncomingParamsObject) -> [ConstellagentRateLimitBucket] {
    ...
    func decodeRateLimitBucket(limitId explicitLimitId: String?, value: JSONValue) -> ConstellagentRateLimitBucket? {
        ...
        return ConstellagentRateLimitBucket(
            limitId: limitId,
    ...
    func decodeDirectRateLimitBuckets(from object: IncomingParamsObject) -> [ConstellagentRateLimitBucket] {
        var buckets: [ConstellagentRateLimitBucket] = []

        if let primary = decodeRateLimitWindow(value: object["primary"] ?? object["primary_window"]) {
            buckets.append(
                ConstellagentRateLimitBucket(
                    limitId: "primary",
                    limitName: firstStringValue(in: object, keys: ["limitName", "limit_name", "name"]),
                    primary: primary,
                    secondary: nil
    ...
    func decodeRateLimitWindow(value: JSONValue?) -> ConstellagentRateLimitWindow? {
        guard let object = value?.objectValue else { return nil }
        ...
        return ConstellagentRateLimitWindow(
    ...
    func mergeRateLimitBuckets(
        existing: [ConstellagentRateLimitBucket],
        incoming: [ConstellagentRateLimitBucket]
    ) -> [ConstellagentRateLimitBucket] {
        ...
        for bucket in incoming {
            if let current = mergedById[bucket.limitId] {
                mergedById[bucket.limitId] = ConstellagentRateLimitBucket(
