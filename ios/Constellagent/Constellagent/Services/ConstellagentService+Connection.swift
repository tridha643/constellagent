// FILE: ConstellagentService+Connection.swift
// Purpose: Connection lifecycle and initialization handshake.
// Layer: Service
// Exports: ConstellagentService connection APIs
...
extension ConstellagentService {
    ...
    private struct ReceiveErrorDisposition {
        ...
        let connectionRecoveryState: ConstellagentConnectionRecoveryState
    ...
    func connect(
        ...
        guard !isConnecting else {
            ...
            throw ConstellagentServiceError.invalidInput("Connection already in progress")
        }
        ...
        let transport: ConstellagentWebSocketTransport
        do {
            ...
            throw ConstellagentServiceError.invalidInput(friendlyMessage)
        }
        switch transport {
    ...
    func disconnect(preserveReconnectIntent: Bool = false) async {
        ...
        failAllPendingRequests(with: ConstellagentServiceError.disconnected)
    }
    ...
    func syncBridgeKeepMacAwakePreferenceIfNeeded(showFailureInUI: Bool = false) async {
        ...
        let handoffService = DesktopHandoffService(constellagent: self)
    ...
    func initializeSession() async throws {
        ...
        let clientInfo: JSONValue = .object([
            "name": .string("constellagentmobile_ios"),
            "title": .string("Constellagent iOS"),
    ...
    func awaitRuntimeInitializedIfNeeded() async throws {
        guard isConnected else {
            throw ConstellagentServiceError.invalidInput("Connect to runtime first.")
        }
        ...
        throw ConstellagentServiceError.invalidInput("Runtime is still initializing. Wait a moment and retry.")
    }
    ...
    func incompatibleBridgeAppVersionError(from error: Error) -> ConstellagentServiceError? {
        ...
        if !message.isEmpty {
            ...
            promptMessage =
                "This device bridge is running Constellagent \(bridgeVersion), which requires Constellagent iPhone \(minimumSupportedAppVersion) or newer. Update the iPhone app, then reconnect."
        } else if let minimumSupportedAppVersion, !minimumSupportedAppVersion.isEmpty {
            promptMessage =
                "This device bridge requires Constellagent iPhone \(minimumSupportedAppVersion) or newer. Update the iPhone app, then reconnect."
        } else {
            promptMessage = "This device bridge requires a newer Constellagent iPhone app. Update the app, then reconnect."
        }

        bridgeUpdatePrompt = ConstellagentBridgeUpdatePrompt(
            title: "Update Constellagent on your iPhone to reconnect",
        ...
        return .invalidInput("This device bridge requires a newer Constellagent iPhone app. Update the app, then reconnect.")
    }
...
extension ConstellagentService {
    ...
    private func receiveErrorDisposition(
        ...
        let connectionRecoveryState: ConstellagentConnectionRecoveryState = shouldAttemptAutoRecovery
        ...
        for candidateArray in candidateArrays {
            ...
            for entry in candidateArray {
                let modeName = entry.objectValue?["mode"]?.stringValue
                    ...
                    ?? entry.stringValue
                if modeName == ConstellagentCollaborationModeKind.plan.rawValue {
                    return true
                }
            }
        }
    ...
    func validateConnectionURL(_ serverURL: String) throws -> URL {
        guard let url = URL(string: serverURL) else {
            let message = ConstellagentServiceError.invalidServerURL(serverURL).localizedDescription
            lastErrorMessage = message
            throw ConstellagentServiceError.invalidServerURL(serverURL)
        }
    ...
    func userFacingConnectError(error: Error, attemptedURL: String, host: String?) -> String {
        ...
        if nsError.domain == NSURLErrorDomain,
           ...
           requiresLocalNetworkAuthorization(for: URL(string: attemptedURL) ?? URL(fileURLWithPath: "/")) {
            return "Constellagent cannot open the local relay connection on this iPhone. Check Local Network and the app's Wi-Fi/Cellular access in Settings, then retry."
        }
    ...
    func presentConnectionErrorIfNeeded(_ error: Error, fallbackMessage: String? = nil) {
        ...
        if message == ConstellagentServiceError.disconnected.localizedDescription,
    ...
    func retryableSessionUnavailableMessage(for closeCode: NWProtocolWebSocket.CloseCode?) -> String? {
        ...
        return "Trying to reach your saved device. Constellagent will keep retrying. If you restarted the bridge on that device, scan the new QR code."
    }
    ...
    func requestLocalNetworkAuthorizationIfNeeded(for url: URL) async throws {
        ...
        guard status != .denied else {
            let message =
                "Constellagent is not allowed to access your local network. Enable Local Network for Constellagent in iPhone Settings and try again."
            lastErrorMessage = message
            throw ConstellagentServiceError.invalidInput(message)
        }
    }
