// FILE: ConstellagentService+History.swift
// Purpose: Parses thread/read history payloads into normalized timeline messages.
// Layer: Service
// Exports: ConstellagentService history parsing helpers
// Depends on: ConstellagentMessage, JSONValue
...
extension ConstellagentService {
    ...
    func mergeHistoryMessagesOffMainActor(
        existing: [ConstellagentMessage],
        history: [ConstellagentMessage],
        activeThreadIDs: Set<String>,
        runningThreadIDs: Set<String>,
        preferRecentWindow: Bool
    ) async throws -> [ConstellagentMessage] {
        let mergeTask = Task.detached(priority: .userInitiated) { () throws -> [ConstellagentMessage] in
    ...
    func decodeMessagesFromThreadRead(threadId: String, threadObject: [String: JSONValue]) -> [ConstellagentMessage] {
        let baseDate = decodeHistoryBaseDate(from: threadObject)
        let turns = threadObject["turns"]?.arrayValue ?? []

        var offset: TimeInterval = 0
        var result: [ConstellagentMessage] = []
        ...
        for turnValue in turns {
            ...
            for itemValue in items {
                ...
                case "message":
                    ...
                    let mappedRole: ConstellagentMessageRole = role.contains("user") ? .user : .assistant
                ...
                case "plan":
                    ...
                    let isJsonlProgressPlan = itemObject["constellagentJsonlProgressPlan"]?.boolValue == true
                    appendHistoryMessage(
                        to: &result,
                        role: .system,
                        kind: .plan,
    ...
    func decodeTurnTerminalStatesFromThreadRead(_ threadObject: [String: JSONValue]) -> [String: ConstellagentTurnTerminalState] {
        let turns = threadObject["turns"]?.arrayValue ?? []
        var result: [String: ConstellagentTurnTerminalState] = [:]
    ...
    func decodeHistoryBaseDate(from threadObject: [String: JSONValue]) -> Date {
        if let rawCreatedAt = threadObject["createdAt"]?.doubleValue {
            return ConstellagentTimestampParser.decodeUnixTimestamp(rawCreatedAt)
        }
        ...
        if let rawUpdatedAt = threadObject["updatedAt"]?.doubleValue {
            return ConstellagentTimestampParser.decodeUnixTimestamp(rawUpdatedAt)
        }
        ...
        if let rawCreatedAt = threadObject["createdAt"]?.stringValue,
           let parsed = ConstellagentTimestampParser.parseString(rawCreatedAt) {
            return parsed
        }
        ...
        if let rawUpdatedAt = threadObject["updatedAt"]?.stringValue,
           let parsed = ConstellagentTimestampParser.parseString(rawUpdatedAt) {
            return parsed
        }
    ...
    func decodeUnixTimestamp(_ rawValue: Double) -> Date {
        ConstellagentTimestampParser.decodeUnixTimestamp(rawValue)
    }
    ...
    func decodeImageAttachments(from itemObject: [String: JSONValue]) -> [ConstellagentImageAttachment] {
        let contentItems = itemObject["content"]?.arrayValue ?? []
    ...
    func mergeHistoryMessages(_ existing: [ConstellagentMessage], _ history: [ConstellagentMessage]) -> [ConstellagentMessage] {
    ...
    nonisolated static func mergeHistoryMessages(
        _ existing: [ConstellagentMessage],
        _ history: [ConstellagentMessage],
        activeThreadIDs: Set<String>,
        runningThreadIDs: Set<String>
    ) throws -> [ConstellagentMessage] {
        if existing.isEmpty {
            ...
            for index in sorted.indices {
                sorted[index].orderIndex = ConstellagentMessageOrderCounter.next()
            }
    ...
    func decodeHistoryTimestamp(from object: [String: JSONValue]) -> Date? {
        ...
        for key in numericKeys {
            if let value = object[key]?.doubleValue {
                return ConstellagentTimestampParser.decodeUnixTimestamp(value)
            }
            if let value = object[key]?.intValue {
                return ConstellagentTimestampParser.decodeUnixTimestamp(Double(value))
            }
            if let value = object[key]?.stringValue {
                if let parsed = ConstellagentTimestampParser.parseString(value) {
                    return parsed
                }
            }
        }
    ...
    func parseHistoryDateString(_ value: String) -> Date? {
        ConstellagentTimestampParser.parseString(value)
    }

    func reconcileExistingMessage(_ localMessage: ConstellagentMessage, with serverMessage: ConstellagentMessage) -> ConstellagentMessage {
        let activeThreadIDs = Set(activeTurnIdByThread.keys)
        let runningIDs = runningThreadIDs
    ...
    nonisolated static func reconcileExistingMessage(
        _ localMessage: ConstellagentMessage,
        with serverMessage: ConstellagentMessage,
        activeThreadIDs: Set<String>,
        runningThreadIDs: Set<String>
        ...
        if ConstellagentTimestampParser.isTrustworthyServerDate(serverMessage.createdAt),
    ...
    nonisolated static func historyMessageKey(for message: ConstellagentMessage) -> String {
    ...
    nonisolated static func stableHistoryTextFingerprint(for text: String) -> String {
        return ConstellagentTextContentFingerprint.cacheKey(for: text)
    }
    ...
    nonisolated static func assistantHistoryIdentityAllowsRunningReconcile(
        localMessage: ConstellagentMessage,
        serverMessage: ConstellagentMessage
    ) -> Bool {
    ...
    nonisolated static func uniqueAssistantHistoryTextMergeIndex(
        in messages: [ConstellagentMessage],
        message: ConstellagentMessage,
        turnId: String
    ) -> Int? {
    ...
    nonisolated static func hasStableToolActivityIdentity(_ value: String?) -> Bool {
        ...
        return !(value.hasPrefix("turn:") && value.contains("|kind:\(ConstellagentMessageKind.toolActivity.rawValue)"))
    }

    // Treats only streaming/skeleton tool rows as safe to rebind by text alone.
    nonisolated static func isProvisionalToolActivityRow(_ message: ConstellagentMessage) -> Bool {
    ...
    nonisolated static func attachmentSignature(for attachments: [ConstellagentImageAttachment]) -> String {
        attachments
    ...
    nonisolated static func shouldReconcileUserHistoryMessage(
        _ candidate: ConstellagentMessage,
        with message: ConstellagentMessage,
        turnId: String
    ) -> Bool {
    ...
    nonisolated static func uniqueUserHistoryMergeIndex(
        in merged: [ConstellagentMessage],
        message: ConstellagentMessage,
        turnId: String
    ) -> Int? {
    ...
    // Centralizes history-item -> ConstellagentMessage mapping without changing ordering behavior.
    func appendHistoryMessage(
        to result: inout [ConstellagentMessage],
        ...
        result.append(
            ConstellagentMessage(
                ...
                proposedPlan: role == .assistant && text.utf8.count <= Self.historyLargeTextByteLimit
                    ? ConstellagentProposedPlanParser.parse(from: text)
                    : nil,
                subagentAction: subagentAction
            )
        )
    ...
    nonisolated static func historyMessagesMergingGeneratedImageArtifacts(_ messages: [ConstellagentMessage]) -> [ConstellagentMessage] {
        var result = messages
    ...
    nonisolated static func isHistoryGeneratedImageArtifactOnly(_ text: String) -> Bool {
        ...
        guard !imageReferences.isEmpty,
              imageReferences.allSatisfy(\.isConstellagentGeneratedImage) else {
            return false
        }
    ...
    func decodeHistoryPlanState(from itemObject: [String: JSONValue]) -> ConstellagentPlanState? {
        let explanation = decodeHistoryNormalizedPlanText(itemObject["explanation"])
            ?? decodeHistoryNormalizedPlanText(itemObject["summary"])
        let steps = (itemObject["plan"]?.arrayValue ?? []).compactMap { stepValue -> ConstellagentPlanStep? in
            guard let stepObject = stepValue.objectValue,
                  ...
                  let status = ConstellagentPlanStepStatus(wireValue: rawStatus) else {
                return nil
            }

            return ConstellagentPlanStep(step: step, status: status)
        }
        ...
        return ConstellagentPlanState(explanation: explanation, steps: steps)
    }

    // Closed turns should not restore a stale "active" plan accessory from history.
    func finalizedHistoryPlanState(_ planState: ConstellagentPlanState?, turnCompleted: Bool) -> ConstellagentPlanState? {
        guard turnCompleted,
              let planState,
        ...
        return ConstellagentPlanState(
            explanation: planState.explanation,
            steps: planState.steps.map { step in
                ConstellagentPlanStep(id: step.id, step: step.step, status: .completed)
            }
        )
    }
    ...
    func historyTurnTerminalState(_ turnObject: [String: JSONValue]) -> ConstellagentTurnTerminalState? {
    ...
    func decodeSubagentActionItem(from itemObject: [String: JSONValue]) -> ConstellagentSubagentAction? {
        ...
        return ConstellagentSubagentAction(
            tool: tool,
            status: status,
            prompt: prompt,
            model: model,
    ...
    private func extractSubagentIdentity(from object: [String: JSONValue]) -> ConstellagentSubagentIdentityEntry {
        ...
        return ConstellagentSubagentIdentityEntry(
    ...
    private func decodeSubagentReceiverThreadIDs(from itemObject: [String: JSONValue]) -> [String] {
        ...
        // Fallback: singular top-level field (Constellagent CLI sends one event per agent).
    ...
    private func decodeSubagentReceiverAgents(
        ...
        return values.enumerated().compactMap { index, value in
            ...
            return ConstellagentSubagentRef(
                threadId: threadId,
    ...
    private func decodeSubagentAgentStates(from itemObject: [String: JSONValue]) -> [String: ConstellagentSubagentState] {
        let candidate = firstValue(
        ...
        if let object = candidate?.objectValue {
            var decoded: [String: ConstellagentSubagentState] = [:]
            for (rawThreadId, value) in object {
                ...
                decoded[threadId] = ConstellagentSubagentState(
                    threadId: threadId,
    ...
    // Builds a single-element agent ref array from top-level fields when the Constellagent CLI sends
    ...
    func decodeHistoryToolCallItem(from itemObject: [String: JSONValue]) -> (kind: ConstellagentMessageKind, text: String)? {
