...
// Depends on: Foundation, ConstellagentService, GitActionsService
...
enum WorktreeFlowCoordinator {
    ...
    static func startNewLocalChat(
        ...
        try await constellagent.startThreadIfReady(preferredProjectPath: preferredProjectPath)
    }
    ...
    static func startNewWorktreeChat(
        ...
        let gitService = GitActionsService(constellagent: constellagent, workingDirectory: normalizedPreferredProjectPath)
        ...
        do {
            return try await constellagent.startThreadIfReady(preferredProjectPath: result.worktreePath)
        } catch {
            let cleanupResult = await cleanupResultForFailedNewWorktreeChat(result, error: error, constellagent: constellagent)
            throw WorktreeFlowError(
    ...
    static func handoffThreadToWorktree(
        ...
        let gitService = GitActionsService(constellagent: constellagent, workingDirectory: normalizedSourceProjectPath)
        let result = try await gitService.createManagedWorktree(
            baseBranch: baseBranch,
            changeTransfer: .move
        )
    ...
    static func handoffThreadToLocal(
        ...
        let gitService = GitActionsService(constellagent: constellagent, workingDirectory: sourceProjectPath)
        var localCheckoutPath: String?
        var didTransferTrackedChanges = false

        do {
            let branches = try await gitService.branchesWithStatus()
            guard let normalizedLocalCheckoutPath = ConstellagentThreadStartProjectBinding.normalizedProjectPath(
            ...
            let movedThread = try await constellagent.moveThreadToProjectPath(
                threadId: thread.id,
    ...
    static func forkThreadToLocal(
        sourceThread: ConstellagentThread,
        ...
        return try await constellagent.forkThreadIfReady(
    ...
    static func forkThreadToWorktree(
        ...
        do {
            try await awaitPreparedWorktreeForkReadiness(constellagent: constellagent)
        } catch {
            let cleanupResult = await cleanupResultForAbortedPreparedWorktreeFork(result, constellagent: constellagent)
            throw WorktreeFlowError(
        ...
        do {
            return try await constellagent.forkThread(
                from: sourceThreadId,
                target: .projectPath(result.worktreePath)
            )
        } catch {
            let cleanupResult = await cleanupResultForFailedWorktreeFork(result, error: error, constellagent: constellagent)
            throw WorktreeFlowError(
                failedWorktreeForkMessage(for: error, cleanupResult: cleanupResult)
            )
        }
    ...
    static func liveThreadForCheckedOutElsewhereBranch(
        ...
        guard let normalizedProjectPath = ConstellagentThreadStartProjectBinding.normalizedProjectPath(projectPath) else {
            return nil
        }
        ...
        return matchingLiveThread(
            in: constellagent.threads,
            projectPath: resolvedProjectPath,
            sort: constellagent.sortThreads
        )
    }
    ...
    static func localForkProjectPath(
        for thread: ConstellagentThread,
        localCheckoutPath: String?
    ) -> String? {
...
private extension WorktreeFlowCoordinator {
    static func handoffThreadToProjectPath(
        ...
        do {
            ...
            if isMissingManagedWorktreeTargetError(error) {
                constellagent.rememberAssociatedManagedWorktreePath(nil, for: threadID)
    ...
    static func awaitPreparedWorktreeForkReadiness(constellagent: ConstellagentService) async throws {
        try await constellagent.awaitRuntimeInitializedIfNeeded()
    }
    ...
    static func recoverFailedThreadRebind(
        ...
        if didTransferTrackedChanges {
            ...
            let rollbackService = GitActionsService(constellagent: constellagent, workingDirectory: reboundProjectPath)
            do {
        ...
        if cleanupManagedWorktreeOnFailedRebind,
           ...
           let reboundProjectPath {
            let cleanupService = GitActionsService(constellagent: constellagent, workingDirectory: reboundProjectPath)
            do {
    ...
    static func cleanupResultForFailedNewWorktreeChat(
        ...
        case .cleanupSafe:
            ...
            let cleanupService = GitActionsService(constellagent: constellagent, workingDirectory: result.worktreePath)
            do {
                try await cleanupService.removeManagedWorktree(branch: nil)
                return .removed
            } catch {
    ...
    static func failedWorktreeForkDisposition(for error: Error) -> WorktreeFlowCleanupDisposition {
        ...
        case .invalidInput(let reason):
            ...
            if normalizedReason.contains("does not support native thread forks yet")
                || normalizedReason.contains("update constellagent on your mac")
