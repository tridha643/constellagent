// FILE: ConstellagentService+ThreadProjectRouting.swift
// Purpose: Keeps thread-to-project routing helpers separate from broader turn lifecycle code.
// Layer: Service Extension
// Exports: ConstellagentService thread project routing helpers

import Foundation

extension ConstellagentService {
    // Reuses the same runtime-readiness gate across every UI entry point that starts a new chat.
    // When no project path is provided we ask the bridge to mint a Constellagent-Desktop-style rootless
    // chat root (`~/Documents/Constellagent/<DATE>/<slug>`) so the persisted thread lands under the
    ...
    @discardableResult
    func moveThreadToProjectPath(threadId: String, projectPath: String) async throws -> ConstellagentThread {
        ...
        guard let normalizedProjectPath = ConstellagentThreadStartProjectBinding.normalizedProjectPath(projectPath) else {
            throw ConstellagentServiceError.invalidInput("A valid project path is required.")
        }
        guard var currentThread = thread(for: normalizedThreadId) else {
            throw ConstellagentServiceError.invalidInput("Thread not found.")
        }
        ...
        if ConstellagentThread.projectIconSystemName(for: normalizedProjectPath) == "arrow.triangle.branch" {
    ...
    func adoptManagedWorktreeProjectPathIfNeeded(threadId: String, projectPath: String?) -> Bool {
        guard let normalizedThreadId = normalizedInterruptIdentifier(threadId),
              let observedProjectPath = ConstellagentThreadStartProjectBinding.normalizedProjectPath(projectPath),
        ...
        guard canonicalCurrentPath == canonicalObservedPath,
              ConstellagentThread.projectIconSystemName(for: canonicalObservedPath) == "arrow.triangle.branch" else {
            return false
        }
...
private extension ConstellagentService {
    ...
    func normalizedStoredProjectPath(_ value: String?) -> String? {
        ConstellagentThreadStartProjectBinding.normalizedProjectPath(value)
    }
}
