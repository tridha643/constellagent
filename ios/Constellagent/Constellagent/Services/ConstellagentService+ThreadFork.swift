// FILE: ConstellagentService+ThreadFork.swift
// Purpose: Owns native thread fork requests and keeps conversation branching separate from handoff/worktree routing.
// Layer: Service
// Exports: ConstellagentService thread fork APIs
// Depends on: Foundation, ConstellagentThread, JSONValue
...
extension ConstellagentService {
    ...
    func forkThreadIfReady(
        ...
        target: ConstellagentThreadForkTarget
    ...
    func forkThread(
        ...
        guard !normalizedSourceThreadId.isEmpty else {
            throw ConstellagentServiceError.invalidInput("A source thread id is required.")
        }
        ...
        do {
            ...
            if consumeUnsupportedThreadFork(error) {
                throw ConstellagentServiceError.invalidInput(
                    "This device bridge does not support native thread forks yet. Update Constellagent on your device and retry."
                )
            }
            throw error
        }
...
private extension ConstellagentService {
    ...
    func resolvedForkProjectPath(
        for target: ConstellagentThreadForkTarget,
        ...
        case .projectPath(let rawPath):
            return ConstellagentThreadStartProjectBinding.normalizedProjectPath(rawPath)
        }
    }
    ...
    func handleThreadForkResponse(
        ...
        guard let resultObject = response.result?.objectValue,
              ...
              var decodedThread = decodeModel(ConstellagentThread.self, from: threadValue) else {
            throw ConstellagentServiceError.invalidResponse("thread/fork response missing thread")
        }
        ...
        if let targetProjectPath {
            ...
            let responseProjectPath = ConstellagentThreadStartProjectBinding.normalizedProjectPath(
    ...
    func hydrateForkedThread(
        ...
        for delay in Self.forkHydrationRetryDelays {
            ...
            let resumedThread: ConstellagentThread?
            do {
