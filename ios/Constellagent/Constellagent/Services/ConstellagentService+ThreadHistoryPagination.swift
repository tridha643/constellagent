// FILE: ConstellagentService+ThreadHistoryPagination.swift
// Purpose: Owns paginated thread history fetch, cursor state, and older-message reveal helpers.
// Layer: Service extension
// Exports: ConstellagentService thread history pagination APIs
// Depends on: ConstellagentService transport, ConstellagentMessage history merge helpers, JSONValue
...
enum TimelineTextClippingPolicy {
    ...
    static func clippedText(for message: ConstellagentMessage, text: String) -> String {
    ...
    static func displayWindow(for message: ConstellagentMessage, text: String, expansionLevel: Int) -> DisplayWindow {
    ...
    private static func byteLimit(for message: ConstellagentMessage) -> Int {
...
extension ConstellagentService {
    // Fetches one cursor page from Constellagent app-server and keeps the response shape tolerant.
    private func fetchThreadTurnsHistoryPage(
        ...
        guard let resultObject = response.result?.objectValue else {
            throw ConstellagentServiceError.invalidResponse("thread/turns/list response missing payload")
        }
        let turns =
        ...
        guard let turns else {
            throw ConstellagentServiceError.invalidResponse("thread/turns/list response missing data array")
        }
    ...
    func fetchLegacyThreadHistoryObject(threadId: String) async throws -> RPCObject {
        ...
        guard let threadObject = response.result?.objectValue?["thread"]?.objectValue else {
            throw ConstellagentServiceError.invalidResponse("thread/read response missing thread payload")
        }
        return threadObject
    }
    ...
    private func olderHistoryMessagesFilteredAndOrderedBeforeExisting(
        _ olderMessages: [ConstellagentMessage],
        existingMessages: [ConstellagentMessage]
    ...
    func persistThreadHistoryPaginationState() {
        ...
        let stateByThreadID = threadIDs.reduce(into: [String: ConstellagentThreadHistoryPaginationState]()) { partial, threadId in
            ...
            partial[threadId] = ConstellagentThreadHistoryPaginationState(
    ...
    func shouldTrustExistingCacheAsPrePaginationFullHistory(
        threadId: String,
        existingMessages: [ConstellagentMessage],
        paginatedMessages: [ConstellagentMessage],
    ...
    private func localCacheStartsAtThreadCreation(threadId: String, existingMessages: [ConstellagentMessage]) -> Bool {
        guard let threadCreatedAt = thread(for: threadId)?.createdAt,
              ConstellagentTimestampParser.isTrustworthyServerDate(threadCreatedAt),
              let oldestMessageDate = existingMessages.map(\.createdAt).min(),
              ConstellagentTimestampParser.isTrustworthyServerDate(oldestMessageDate) else {
            return false
        }
    ...
    func shouldDeferThreadHistoryAfterTimeout(_ error: ConstellagentServiceError) -> Bool {
