// FILE: ConstellagentService.swift
// Purpose: Central state container for Constellagent app-server communication.
// Layer: Service
// Exports: ConstellagentService, ConstellagentApprovalRequest
// Depends on: Foundation, Observation, RPCMessage, ConstellagentThread, ConstellagentMessage, UserNotifications
...
struct ConstellagentApprovalRequest: Identifiable, Sendable {
    let id: String
...
struct ConstellagentRecentActivityLine {
    let line: String
    let timestamp: Date
}

struct ConstellagentRunningThreadWatch: Equatable, Sendable {
    let threadId: String
    let expiresAt: Date
}

struct ConstellagentThreadResumeRequestSignature: Equatable, Sendable {
    let projectPath: String?
    let modelIdentifier: String?
}

struct ConstellagentThreadHistoryPaginationState: Codable, Equatable, Sendable {
    var olderCursor: JSONValue?
    var exhaustedOlderCursor: JSONValue?
    var hasAuthoritativeLocalHistoryStart: Bool
}

struct ConstellagentSubagentIdentityEntry: Equatable, Sendable {
    var threadId: String?
    var agentId: String?
    var nickname: String?
    var role: String?
...
struct ConstellagentSecureControlWaiter {
    let id: UUID
    let continuation: CheckedContinuation<String, Error>
}

enum ConstellagentWebSocketTransport {
    case network(NWConnection)
    case manualTCP(NWConnection)
    case urlSession(URLSession, URLSessionWebSocketTask)
}

final class ConstellagentURLSessionWebSocketDelegate: NSObject, URLSessionWebSocketDelegate, URLSessionTaskDelegate {
    ...
    func urlSession(
        ...
        if closeCode == .invalid {
            resolveOpen(with: .failure(ConstellagentServiceError.disconnected))
            return
        }

        resolveOpen(
            with: .failure(
                ConstellagentServiceError.invalidInput("WebSocket closed during connect (\(closeCode.rawValue))")
            )
        )
    }
...
struct ConstellagentBridgeUpdatePrompt: Identifiable, Equatable, Sendable {
    let id = UUID()
    let title: String
    let message: String
    let command: String?
...
struct ConstellagentThreadRuntimeOverride: Codable, Equatable, Sendable {
    var reasoningEffort: String?
    var serviceTierRawValue: String?
    var overridesReasoning: Bool
    var overridesServiceTier: Bool

    var serviceTier: ConstellagentServiceTier? {
        guard let serviceTierRawValue else {
            return nil
        }
        return ConstellagentServiceTier(rawValue: serviceTierRawValue)
    }
...
struct ConstellagentThreadCompletionBanner: Identifiable, Equatable, Sendable {
    let id = UUID()
    let threadId: String
    let title: String
}

struct ConstellagentMissingNotificationThreadPrompt: Identifiable, Equatable, Sendable {
    let id = UUID()
    let threadId: String
}

enum ConstellagentThreadRunBadgeState: Hashable, Sendable {
    case running
    case ready
    case failed
}

enum ConstellagentRunCompletionResult: String, Equatable, Sendable {
    case completed
    case failed
}

enum ConstellagentNotificationPayloadKeys {
...
enum ConstellagentTurnTerminalState: String, Codable, Equatable, Sendable {
    case completed
    case failed
    case stopped
}

enum ConstellagentConnectionRecoveryState: Equatable, Sendable {
    case idle
    case retrying(attempt: Int, message: String)
}

enum ConstellagentConnectionPhase: Equatable, Sendable {
    case offline
    case connecting
    case loadingChats
    case syncing
...
enum ConstellagentPendingThreadComposerAction: Equatable, Sendable {
    case codeReview(target: ConstellagentPendingCodeReviewTarget)
}

enum ConstellagentThreadForkTarget: Equatable, Sendable {
    case currentProject
    case projectPath(String)
}

enum ConstellagentPendingCodeReviewTarget: Equatable, Sendable {
...
struct TurnTimelineRenderSnapshot: Equatable {
    let threadID: String
    let messages: [ConstellagentMessage]
    let messageIndexByID: [String: Int]
    let planMatchingMessages: [ConstellagentMessage]
    let timelineChangeToken: Int
    let activeTurnID: String?
    let isThreadRunning: Bool
    let latestTurnTerminalState: ConstellagentTurnTerminalState?
...
struct PendingSystemStreamingDeltas {
    ...
    let turnId: String?
    let itemId: String
    let kind: ConstellagentMessageKind
    var deltas: [String]
}
...
final class ThreadTimelineState {
    let threadID: String
    var messages: [ConstellagentMessage]
    var messageRevision: Int
    var activeTurnID: String?
    var isThreadRunning: Bool
    var latestTurnTerminalState: ConstellagentTurnTerminalState?
...
@MainActor
@Observable
final class ConstellagentService {
    static let minimumSupportedBridgePackageVersion = "1.3.9"

    // --- Public state ---------------------------------------------------------

    var threads: [ConstellagentThread] = [] {
        didSet {
    ...
    var messagesByThread: [String: [ConstellagentMessage]] = [:]
    ...
    var availableModels: [ConstellagentModelOption] = []
    var selectedModelId: String?
    var hasPersistedSelectedModelId = false
    var selectedGitWriterModelId: String?
    var selectedReasoningEffort: String?
    var selectedServiceTier: ConstellagentServiceTier?
    // Per-chat runtime overrides let the composer diverge from app-wide defaults.
    var threadRuntimeOverridesByThreadID: [String: ConstellagentThreadRuntimeOverride] = [:]
    var selectedAccessMode: ConstellagentAccessMode = .onRequest
    // Bridge-owned ChatGPT auth snapshot used by Settings and voice gating.
    var gptAccountSnapshot: ConstellagentGPTAccountSnapshot = constellagentGPTAccountInitialSnapshot() {
        didSet {
    ...
    var pendingComposerActionByThreadID: [String: ConstellagentPendingThreadComposerAction] = [:]
    ...
    var relayProtocolVersion: Int = constellagentSecureProtocolVersion
    ...
    var secureConnectionState: ConstellagentSecureConnectionState = .notPaired
    ...
    var bridgeUpdatePrompt: ConstellagentBridgeUpdatePrompt?
    ...
    var threadCompletionBanner: ConstellagentThreadCompletionBanner?
    // Explains why a push-opened chat could not be restored and offers a recovery path.
    var missingNotificationThreadPrompt: ConstellagentMissingNotificationThreadPrompt?
    // Interactive SSH terminal state is owned on-device so it can bootstrap a Mac before the bridge runs.
    var terminalSnapshot: ConstellagentTerminalSnapshot = .idle
    var terminalSnapshotsById: [String: ConstellagentTerminalSnapshot] = [:]
    var terminalProfile: ConstellagentTerminalProfile = ConstellagentTerminalProfileStore.load()
    @ObservationIgnored let nativeSSHTerminal = ConstellagentNativeSSHTerminal()
    @ObservationIgnored var nativeSSHTerminalsById: [String: ConstellagentNativeSSHTerminal] = [:]

    // --- Internal wiring ------------------------------------------------------

    var webSocketConnection: NWConnection?
    var webSocketSession: URLSession?
    var webSocketSessionDelegate: ConstellagentURLSessionWebSocketDelegate?
    ...
    let webSocketQueue = DispatchQueue(label: "Constellagent.WebSocket", qos: .userInitiated)
    var pendingRequests: [String: CheckedContinuation<RPCMessage, Error>] = [:]
    @ObservationIgnored var pendingRequestDebugMeta: [String: (method: String, startedAt: Date)] = [:]
    @ObservationIgnored var assistantDeltaDebugSequenceByThread: [String: Int] = [:]
    ...
    @ObservationIgnored var trustedSessionResolverOverride: (() async throws -> ConstellagentTrustedSessionResolveResponse)?
    ...
    @ObservationIgnored var trustedSessionResolveTask: Task<ConstellagentTrustedSessionResolveResponse, Error>?
    ...
    var recentActivityLineByThread: [String: ConstellagentRecentActivityLine] = [:]
    var contextWindowUsageByThread: [String: ContextWindowUsage] = [:]
    var rateLimitBuckets: [ConstellagentRateLimitBucket] = []
    ...
    @ObservationIgnored var threadResumeTaskByThreadID: [String: Task<ConstellagentThread?, Error>] = [:]
    // Remembers which cwd/model pair an in-flight resume is actually targeting.
    @ObservationIgnored var threadResumeRequestSignatureByThreadID: [String: ConstellagentThreadResumeRequestSignature] = [:]
    ...
    @ObservationIgnored var threadListFetchTaskByLimit: [Int: (id: UUID, task: Task<[ConstellagentThread], Error>)] = [:]
    ...
    // Tracks whether the bridge is proxying a real Constellagent endpoint or a spawned local app-server.
    var constellagentTransportMode: ConstellagentRuntimeTransportMode = .unknown
    var bridgeHostPlatform: ConstellagentBridgeHostPlatform {
        if let hostPlatform = gptAccountSnapshot.hostPlatform {
            return hostPlatform
        }
        return preferredTrustedMacRecord == nil ? .unknown : .macOS
    }
    var bridgeHostCapabilities: ConstellagentBridgeHostCapabilities {
    ...
    var planSessionSourceByThread: [String: ConstellagentPlanSessionSource] = [:] {
        didSet {
            persistPlanSessionSources()
        }
    }
    var runningThreadWatchByID: [String: ConstellagentRunningThreadWatch] = [:]
    ...
    var notificationCenterDelegateProxy: ConstellagentNotificationCenterDelegateProxy?
    ...
    var secureSession: ConstellagentSecureSession?
    var pendingHandshake: ConstellagentPendingHandshake?
    var phoneIdentityState: ConstellagentPhoneIdentityState
    var trustedMacRegistry: ConstellagentTrustedMacRegistry
    ...
    var pendingSecureControlContinuations: [String: [ConstellagentSecureControlWaiter]] = [:]
    ...
    @ObservationIgnored var threadByID: [String: ConstellagentThread] = [:]
    @ObservationIgnored var threadIndexByID: [String: Int] = [:]
    @ObservationIgnored var firstLiveThreadIDCache: String?
    @ObservationIgnored var subagentIdentityByThreadID: [String: ConstellagentSubagentIdentityEntry] = [:]
    @ObservationIgnored var subagentIdentityByAgentID: [String: ConstellagentSubagentIdentityEntry] = [:]
    ...
    @ObservationIgnored var pinnedThreadSnapshotsByRootID: [String: [ConstellagentThread]] = [:]
    ...
    let messagePersistence = ConstellagentMessagePersistence()
    let composerDraftPersistence = ConstellagentComposerDraftPersistence()
    let aiChangeSetPersistence = AIChangeSetPersistence()
    let defaults: UserDefaults
    let userNotificationCenter: ConstellagentUserNotificationCentering
    var remoteNotificationRegistrar: ConstellagentRemoteNotificationRegistering?

    static let selectedModelIdDefaultsKey = "constellagent.selectedModelId"
    static let selectedGitWriterModelIdDefaultsKey = "constellagent.selectedGitWriterModelId"
    static let selectedReasoningEffortDefaultsKey = "constellagent.selectedReasoningEffort"
    static let selectedServiceTierDefaultsKey = "constellagent.selectedServiceTier"
    static let threadRuntimeOverridesDefaultsKey = "constellagent.threadRuntimeOverrides"
    static let planSessionSourcesDefaultsKey = "constellagent.planSessionSources"
    static let selectedAccessModeDefaultsKey = "constellagent.selectedAccessMode"
    static let locallyArchivedThreadIDsKey = "constellagent.locallyArchivedThreadIDs"
    static let locallyDeletedThreadIDsKey = "constellagent.locallyDeletedThreadIDs"
    static let forkedThreadOriginsDefaultsKey = "constellagent.forkedThreadOrigins"
    static let renamedThreadNamesDefaultsKey = "constellagent.renamedThreadNames"
    static let pinnedThreadIDsDefaultsKey = "constellagent.pinnedThreadIDs"
    static let pinnedThreadSnapshotsDefaultsKey = "constellagent.pinnedThreadSnapshots"
    static let associatedManagedWorktreePathsDefaultsKey = "constellagent.associatedManagedWorktreePaths"
    static let turnTerminalStatesDefaultsKey = "constellagent.turnTerminalStates"
    static let threadHistoryPaginationStateDefaultsKey = "constellagent.threadHistoryPaginationState"
    static let notificationsPromptedDefaultsKey = "constellagent.notifications.prompted"
    static let keepMacAwakeWhileBridgeRunsDefaultsKey = "constellagent.keepMacAwakeWhileBridgeRuns"

    init(
        encoder: JSONEncoder = JSONEncoder(),
        decoder: JSONDecoder = JSONDecoder(),
        defaults: UserDefaults = .standard,
        userNotificationCenter: ConstellagentUserNotificationCentering? = nil,
        remoteNotificationRegistrar: ConstellagentRemoteNotificationRegistering? = nil
    ) {
        self.encoder = encoder
        self.decoder = decoder
        self.defaults = defaults
        self.userNotificationCenter = userNotificationCenter ?? UNUserNotificationCenter.current()
        self.remoteNotificationRegistrar = remoteNotificationRegistrar ?? ConstellagentApplicationRemoteNotificationRegistrar()
        self.phoneIdentityState = constellagentPhoneIdentityStateFromSecureStore()
        self.trustedMacRegistry = constellagentTrustedMacRegistryFromSecureStore()
        self.currentTrustedMacDeviceId = SecureStore.readString(for: ConstellagentSecureKeys.currentTrustedMacDeviceId)
        self.lastTrustedMacDeviceId = SecureStore.readString(for: ConstellagentSecureKeys.lastTrustedMacDeviceId)
        ...
        if savedServiceTier == "flex" {
           ...
           let parsedServiceTier = ConstellagentServiceTier(rawValue: savedServiceTier) {
        ...
        if let savedAccessMode = defaults.string(forKey: Self.selectedAccessModeDefaultsKey),
           let parsedAccessMode = ConstellagentAccessMode(rawValue: savedAccessMode) {
            self.selectedAccessMode = parsedAccessMode
        } else {
            self.selectedAccessMode = .onRequest
        }

        self.gptAccountSnapshot = constellagentGPTAccountInitialSnapshot()

        // Restore relay session from Keychain
        self.relaySessionId = SecureStore.readString(for: ConstellagentSecureKeys.relaySessionId)
        self.relayUrl = SecureStore.readString(for: ConstellagentSecureKeys.relayUrl)
        self.relayMacDeviceId = SecureStore.readString(for: ConstellagentSecureKeys.relayMacDeviceId)
        self.relayMacIdentityPublicKey = SecureStore.readString(for: ConstellagentSecureKeys.relayMacIdentityPublicKey)
        if let rawProtocolVersion = SecureStore.readString(for: ConstellagentSecureKeys.relayProtocolVersion),
           let parsedProtocolVersion = Int(rawProtocolVersion) {
            self.relayProtocolVersion = parsedProtocolVersion
        } else {
            self.relayProtocolVersion = constellagentSecureProtocolVersion
        }
        if let rawLastAppliedSeq = SecureStore.readString(for: ConstellagentSecureKeys.relayLastAppliedBridgeOutboundSeq),
        ...
        self.remoteNotificationDeviceToken = SecureStore.readString(for: ConstellagentSecureKeys.pushDeviceToken)
        if let relayMacDeviceId,
           let trustedMac = trustedMacRegistry.records[relayMacDeviceId] {
            self.secureConnectionState = .trustedMac
            self.secureMacFingerprint = constellagentSecureFingerprint(for: trustedMac.macIdentityPublicKey)
    ...
    func startNetworkPathMonitor() {
        ...
        monitor.start(queue: DispatchQueue(label: "Constellagent.NetworkPathMonitor", qos: .utility))
    }
    ...
    var preferredTrustedMacRecord: ConstellagentTrustedMacRecord? {
    ...
    var currentTrustedMacRecord: ConstellagentTrustedMacRecord? {
        guard let normalizedCurrentTrustedMacDeviceId else {
            return nil
        }
    ...
    func trustedMacRecord(for deviceId: String?) -> ConstellagentTrustedMacRecord? {
        guard let normalizedDeviceId = deviceId?
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .constellagentNilIfEmpty else {
            return nil
        }
    ...
    var connectionPhase: ConstellagentConnectionPhase {
        if isConnecting {
            return .connecting
        }
    ...
    func setCurrentTrustedMacDeviceId(_ deviceId: String?) {
        ...
        if let normalizedDeviceId {
            SecureStore.writeString(normalizedDeviceId, for: ConstellagentSecureKeys.currentTrustedMacDeviceId)
        } else {
...
private extension String {
    var constellagentNilIfEmpty: String? {
        isEmpty ? nil : self
    }
}
