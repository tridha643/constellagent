// FILE: ConstellagentService+TurnPaginationCompatibility.swift
// Purpose: Detects runtimes that cannot page thread turns and centralizes fallback state.
// Layer: Service
// Exports: ConstellagentService turn-pagination compatibility helpers
// Depends on: Foundation, RPCMessage, ConstellagentServiceError
...
extension ConstellagentService {
    ...
    func learnTurnPaginationSupportFromInitializeResponse(_ response: RPCMessage) {
        ...
        guard let version = constellagentCLIUserAgentVersion(userAgent),
              constellagentVersion(version, isOlderThan: (0, 125, 0)) else {
            return
        }
...
private extension ConstellagentService {
    // Litter uses constellagent_cli_rs/0.125.0 as the minimum app-server turn-pagination version.
    func constellagentCLIUserAgentVersion(_ userAgent: String) -> (major: Int, minor: Int, patch: Int)? {
        let parts = userAgent.split(separator: "/", maxSplits: 1)
        guard parts.count == 2,
              parts[0] == "constellagent_cli_rs" else {
            return nil
        }
