// FILE: ConstellagentService+Review.swift
// Purpose: Starts Constellagent reviewer runs and applies the same optimistic message bookkeeping as chat sends.
// Layer: Service
// Exports: ConstellagentService review-start helpers
// Depends on: ConstellagentService, RPCMessage

import Foundation

enum ConstellagentReviewTarget {
...
private struct ReviewStartRequest {
    ...
    let target: ConstellagentReviewTarget
...
extension ConstellagentService {
    ...
    func startReview(
        threadId: String,
        target: ConstellagentReviewTarget?,
        baseBranch: String? = nil
    ) async throws {
        guard let target else {
            throw ConstellagentServiceError.invalidInput("Choose a review target first.")
        }
    ...
    // Uses the review/start target schema expected by the local Constellagent runtime.
    private func buildReviewStartParams(
        ...
        case .baseBranch:
            ...
            guard let resolvedBaseBranch = normalizedBaseBranch,
                ...
                throw ConstellagentServiceError.invalidInput("Choose a base branch before starting this review.")
            }
            targetObject = [
    ...
    private func reviewPromptText(target: ConstellagentReviewTarget, baseBranch: String?) -> String {
        switch target {
        case .uncommittedChanges:
