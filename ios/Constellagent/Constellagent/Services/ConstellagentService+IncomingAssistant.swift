// FILE: ConstellagentService+IncomingAssistant.swift
// Purpose: Handles assistant-specific incoming events (delta/start/completed) and identity normalization.
// Layer: Service
// Exports: ConstellagentService assistant incoming handlers
...
extension ConstellagentService {
    ...
    func appendCompletedAgentText(from paramsObject: IncomingParamsObject?) {
        ...
        guard let itemObject else {
            // Some legacy constellagent/event notifications carry only plain final message text.
...
private extension ConstellagentService {
    // Extracts assistant delta text across stable + legacy constellagent/event envelopes.
    func extractAssistantDeltaText(
        ...
        let candidates = [
            ...
            eventObject?["summary"]?.stringValue,
            eventObject?["part"]?.stringValue,
            paramsObject["event"]?.objectValue?["delta"]?.stringValue,
            paramsObject["event"]?.objectValue?["text"]?.stringValue,
        ]
        guard let delta = candidates.compactMap({ value -> String? in
    ...
    // Constellagent app-server can emit final_answer text before task_complete without
    ...
    // Legacy constellagent/event assistant notifications can encode turn id in params.id.
