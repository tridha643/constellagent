// FILE: ConstellagentService+IncomingSupport.swift
// Purpose: Shared parsing helpers and lightweight models used by inbound event handling.
// Layer: Service support
// Exports: Incoming decode helpers for ConstellagentService inbound routing
...
func normalizedIncomingMethodName(_ method: String) -> String {
    ...
    // The constellagent bridge publishes its own event vocabulary
    // (`session.started`, `session.message.delta`, …) via
    // `desktop/src/main/mobile-event-bridge.ts`. Map them onto the codex-style
    // notification names the upstream iOS dispatchers were built for, so the
    // existing turn/timeline/plan handlers in
    // `ConstellagentService+Incoming*.swift` can continue to match on substrings
    // like "delta", "started", "completed" without per-call edits.
    switch trimmed {
    case "session.started":            return "thread/turn/started"
    case "session.message.delta":      return "thread/turn/message/delta"
    case "session.message.completed":  return "thread/turn/completed"
    case "session.error":               return "thread/turn/error"
    case "session.plan.emitted":       return "thread/plan/emitted"
    case "session.tool.call":          return "thread/turn/tool_call"
    case "session.tool.result":        return "thread/turn/tool_result"
    case "session.queued":             return "thread/queued"
    case "session.cancelled":          return "thread/turn/cancelled"
    case "workspace.changed":          return "workspace/filechange"
    case "git.status.changed":         return "git/status"
    case "annotation.created":         return "annotation/created"
    case "annotation.resolved":        return "annotation/resolved"
...
func threadTerminalState(from normalizedStatusType: String) -> ConstellagentTurnTerminalState? {
