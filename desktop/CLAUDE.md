...
bun run verify:files-panel    # File-tree e2e slice
```
...
├── <domain>-service.ts × ~40           state, github, graphite,       ├── components/ (~31 dirs)
...
### Main-process service map (`src/main/`, ~140 files incl. tests)

Use this to jump straight to the owning file for a feature:
...
| **Git / worktrees** | `git-service.ts`, `git-snapshot.ts`, `worktree-sync-service.ts`, `worktree-credential-copy.ts` | Worktree CRUD, staging/commit/diff/log, sync across worktrees |
...
| **Agent drivers** | `agents/agent-driver.ts`, `agents/codex-driver*.ts`, `agents/cursor-driver*.ts`, `agents/pi-conductor-driver.ts` | Per-provider CLI/SDK integration (Codex, Cursor, Pi) |
...
| **Automations** | `automation-scheduler.ts`, `automation-engine.ts`, `automation-event-bus.ts`, `automations/*.ts` | Cron + event-driven workspace/agent automations |
| **Composio** | `composio-webhook-service.ts`, `composio-trigger-client.ts`, `composio-ngrok-service.ts`, `composio-*.ts` | Webhook/trigger ingestion for event automations |
...
| **Skills / MCP / LSP** | `skills-service.ts`, `mcp-config.ts`, `mcp-status-service.ts`, `lsp/lsp-service.ts` | Skill/subagent KV catalog, MCP server config (`~/.claude.json`), language servers |
...
| **Linear** | `linear-draft-service.ts` | Linear issue draft generation |
...
| Components | `components/` (~31 subdirs) | `Terminal`, `Editor`, `Sidebar`, `TabBar`, `RightPanel`, `Conductor`, `QuickOpen`, `MarkdownPreview`/`MarkdownRenderer`, `HunkReview`, `Automations`, `PlanPalette`, `Settings`, `ui/` (shadcn/base-ui) … |
| Hooks | `hooks/` (~16) | `useShortcuts`, `usePrStatusPoller`, `useWorktreeSyncPoller`, `useContextWindowPoller`, `useUsageLimitsPoller`, `useGraphiteStackPoller`, `useFileWatcher`, `useGitGutter`, `use-prefers-reduced-motion`, … |
| Conductor GUI | `pi-gui/` | Pi/Conductor agent chat UI + hooks |
| Linear | `linear/`, `LinearWorkspace` component | Linear issue integration |
