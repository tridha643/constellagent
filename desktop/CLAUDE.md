...
Run from the **repo root** (these wrap the `desktop` scripts and first build `constellagent-mobile-protocol`):
...
└── mobile-*.ts (iOS bridge)            agentChat, lsp, mcp, …         └── pi-gui/ (Conductor Pi extension UI)
```
...
| **Pi host** | `pi-host-service.ts`, `pi-run-prompt.ts`, `pi-session-state.ts`, `pi-timeline.ts`, `pi-models.ts`, `pi-app-store-utils.ts`, `lib/pi-gui/` | Pi Conductor backend (`@mariozechner/pi-coding-agent` + vendored `@pi-gui/*`) |
...
| **Agentation** | `agentation-http-server.ts`, `agentation-service.ts` | Embeds Agentation HTTP/SSE server via `agentation-mcp` `startHttpServer` (default `:4747`, store under userData); SSE → `AGENTATION_EVENT`, status/list/resolve/dismiss IPC. Browser tabs inject `<Agentation />` in webview with clipboard/submit bridge. |
