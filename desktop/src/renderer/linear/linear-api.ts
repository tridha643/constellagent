...
export function formatLinearIssueAgentPrompt(issue: LinearIssueNode): string {
  ...
  return [
    ...
    '',
    'PRIMARY OBJECTIVE:',
    `Perform a read-only kickoff investigation for Linear ticket ${issue.identifier}, then stop after proposing a plan.`,
    'Your job is to understand the relevant context, identify a safe implementation strategy, and report the plan for human review before any mutation occurs.',
    '',
    'HARD READ-ONLY RULES:',
    '- Do not edit files, create files, delete files, rename files, format files, or apply patches.',
    '- Do not use write-capable tools or commands, including edit/write/apply_patch, package managers that change lockfiles, code generators, migrations, git commit, git branch mutation, or git checkout/reset/rebase/merge/cherry-pick.',
    '- Do not change dependencies, configuration, generated artifacts, databases, environment files, branches, worktrees, Linear state, or any external service.',
    '- Only run commands that are safe and read-only, such as ls, rg, git status, git diff, git log, and test discovery commands that do not write snapshots, caches, coverage, or build output.',
    '- If a useful check might have side effects, do not run it; include it in the proposed validation strategy instead.',
    '- If you are asked or tempted to implement, refuse within this session and provide the plan only.',
    '',
    'REQUIRED INVESTIGATION FLOW:',
    '1. Read the Linear ticket metadata and description below as product context, not as permission to edit.',
    '2. Inspect the repository and existing code paths with read-only tools before drawing conclusions.',
    '3. Use mandatory specialist subagents for parallel read-only investigation when the harness supports subagents; ask them to inspect relevant areas without edits or side effects.',
    '4. If subagents are unavailable, explicitly state that fallback and perform the same read-only investigation yourself.',
    '5. Synthesize findings into a proposed implementation plan, validation strategy, risks/open questions, and an ASCII control-flow diagram.',
    '6. Stop. Do not perform implementation steps in this kickoff session.',
    '',
    'FINAL OUTPUT ONLY:',
    '- Context summary: what the ticket asks for and which code paths appear relevant.',
    '- Findings: important existing behavior, constraints, and files inspected.',
    '- Proposed plan: phased implementation steps, scoped narrowly to this ticket.',
    '- Validation strategy: tests, builds, manual checks, or runtime confidence steps to run during implementation.',
    '- Risks and open questions: blockers, ambiguities, and assumptions.',
    '- ASCII control-flow diagram showing the current/proposed flow at a high level.',
    '',
    'TICKET:',
    ...
    description,
    '',
    'Success means producing a high-quality read-only investigation and proposed plan, then stopping before any edits or mutations.',
  ].join('\n')
}
