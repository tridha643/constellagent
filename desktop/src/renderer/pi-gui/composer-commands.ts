...
/** Next thinking level in `THINKING_OPTIONS` order; missing/unknown `current` yields `low`. */
export function nextThinkingLevel(current: string | undefined): string {
  const n = THINKING_OPTIONS.length;
  const idx = current ? THINKING_OPTIONS.findIndex((o) => o.value === current) : -1;
  const base = idx < 0 ? -1 : idx;
  return THINKING_OPTIONS[(base + 1) % n]!.value;
}

/**
 * Values for `data-reasoning` on composer chrome.
 * `low` maps to undefined (empty — default focus ring); colored tiers start at medium.
 */
export function reasoningLevelDataAttr(
  level: string | undefined,
): "medium" | "high" | "xhigh" | undefined {
  if (!level) {
    return undefined;
  }
  ...
  const ok = THINKING_OPTIONS.some((o) => o.value === normalized);
  return ok ? (normalized as "medium" | "high" | "xhigh") : undefined;
}

/** Active `/…` token at the caret (slash may appear anywhere after whitespace / start). */
export interface ActiveSlashToken {
  readonly query: string;
  /** Index of `/` in `value` */
  readonly from: number;
  /** Caret position (exclusive end of token) */
  readonly to: number;
}

/**
 * Detects a slash-command being typed at the cursor: `… /plan`, `… /`, etc.
 * Does not trigger inside words or paths (`foo/bar`) — `/` must follow start or whitespace.
 */
...
export function slashOptionEmptyState(
  ...
  if (
    command.kind === "login" &&
    buildProviderOptions(runtime?.providers ?? [], (provider) => provider.oauthSupported).length === 0
  ) {
    return {
      title: "No OAuth providers in runtime",
      description:
        runtime?.providers?.length
          ? "Runtime loaded but no OAuth-capable providers are listed. Refresh the Pi workspace or run pi login in a terminal, then reopen this thread."
          : "Workspace runtime is still loading or failed to load. Wait a moment, switch sessions, or check the main process log.",
    };
  }
...
function providerRank(provider: RuntimeProviderRecord): number {
  ...
  if (provider.id === "openai-codex" || provider.id === "anthropic" || provider.id === "google-antigravity") {
    return 1;
  }
  if (provider.oauthSupported) {
    return 2;
