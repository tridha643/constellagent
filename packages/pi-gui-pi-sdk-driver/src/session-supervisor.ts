...
import { KeybindingsManager, TUI_KEYBINDINGS, type Component, type TUI } from "@mariozechner/pi-tui";
...
import { createHeadlessTerminal } from "./headless-tui-host.js";
import {
...
interface ManagedSessionRecord {
  ...
  >;
  pendingExtensionDialogs: Map<string, ExtensionUiDialogRequest>;
  extensionUiState: ExtensionUiState;
  sessionCommands: RuntimeCommandRecord[];
  tuiCustomBridge: { handleInput: (data: string) => void } | undefined;
}
...
export class SessionSupervisor {
  ...
  /** Live context meter for the open session (no workspace JSON from extensions). */
  getContextUsageSnapshot(sessionRef: SessionRef): PiContextUsageSnapshot | null {
    ...
    const model = formatPiSessionModelLabel(session);

    if (usage && usage.tokens != null && usage.percent != null) {
      return {
        usedTokens: Math.max(0, Math.round(usage.tokens)),
        contextWindowSize: Math.max(1, Math.round(usage.contextWindow)),
        percentage: Math.min(100, Math.max(0, Math.round(usage.percent))),
        model,
    ...
    const usedTokens = stats.tokens.input + stats.tokens.cacheRead + stats.tokens.cacheWrite;
    const contextWindowSize = resolvePiContextWindowSize(usage, session.model, usedTokens);
  ...
  async respondToHostUiRequest(sessionRef: SessionRef, response: HostUiResponse): Promise<void> {
    ...
    record.pendingExtensionDialogs.delete(response.requestId);
    pending.resolve(response);
  }

  deliverExtensionTuiInput(sessionRef: SessionRef, data: string): void {
    const record = this.records.get(sessionKey(sessionRef));
    record?.tuiCustomBridge?.handleInput(data);
  }
  ...
  getSessionExtensionUiSnapshot(sessionRef: SessionRef): PiSessionExtensionUiSnapshot {
    ...
    const tc = record.extensionUiState.tuiCustom;
    const title = record.extensionUiState.title;
    const editorText = record.extensionUiState.editorText;
    return {
      statuses: [...record.extensionUiState.statuses.entries()].map(([key, text]) => ({ key, text })),
      widgets: [...record.extensionUiState.widgets.values()].map((w) => ({
        key: w.key,
        lines: [...w.lines],
        placement: w.placement,
      })),
      pendingDialogs: [...record.pendingExtensionDialogs.values()],
  ...
  private async emitSessionBeforeSwitch(
    ...
    return { cancelled: (result as { cancel?: boolean } | undefined)?.cancel === true };
  }

  private async emitSessionBeforeFork(session: AgentSession, entryId: string): Promise<{ cancelled: boolean }> {
    const runner = session.extensionRunner;
    if (!runner?.hasHandlers("session_before_fork")) {
      return { cancelled: false };
    }
    const result = await runner.emit({ type: "session_before_fork", entryId } as never);
    return { cancelled: (result as { cancel?: boolean } | undefined)?.cancel === true };
  }

  private async replaceLiveAgentSession(
    record: ManagedSessionRecord,
    buildNext: () => Promise<AgentSession>,
  ): Promise<void> {
    const oldRef: SessionRef = { ...record.ref };
    const oldKey = sessionKey(oldRef);
    ...
    const nextSession = await buildNext();
    record.session = nextSession;
    record.sessionFile = nextSession.sessionFile ?? nextSession.sessionManager.getSessionFile();
    record.unsubscribeAgent = nextSession.subscribe((event) => {
      void this.handleAgentEvent(record, event);
    });
    ...
    if (oldKey !== nextKey) {
      await this.catalogs.sessions.deleteSession(oldRef);
  ...
  private createCommandContextActions(record: ManagedSessionRecord): ExtensionCommandContextActions {
    return {
      ...
      newSession: async (options) => {
        const session = this.requireSession(record);
        const before = await this.emitSessionBeforeSwitch(session, "new");
        if (before.cancelled) {
          return { cancelled: true };
        }
        const cwd = session.sessionManager.getCwd();
        const sessionDir = session.sessionManager.getSessionDir();
        await this.replaceLiveAgentSession(record, async () => {
          const sessionManager = SessionManager.create(cwd, sessionDir);
          if (options?.parentSession) {
            sessionManager.newSession({ parentSession: options.parentSession });
          }
          const { session: next } = await this.createAgentSessionImpl({
            cwd: record.workspace.path,
            sessionManager,
            ...(this.modelRegistry ? { modelRegistry: this.modelRegistry } : {}),
          });
          if (options?.setup) {
            await options.setup(next.sessionManager);
            next.agent.state.messages = next.sessionManager.buildSessionContext().messages;
          }
          return next;
        });
        return { cancelled: false };
      ...
      fork: async (entryId) => {
        const session = this.requireSession(record);
        const before = await this.emitSessionBeforeFork(session, entryId);
        if (before.cancelled) {
          return { cancelled: true };
        }
        ...
        if (session.sessionManager.isPersisted()) {
          ...
          if (!selectedEntry.parentId) {
            await this.replaceLiveAgentSession(record, async () => {
              const sessionManager = SessionManager.create(record.workspace.path, sessionDir);
          ...
          const branchedManager = SessionManager.open(forkedSessionPath, sessionDir);
          await this.replaceLiveAgentSession(record, async () => {
            const { session: next } = await this.createAgentSessionImpl({
              cwd: record.workspace.path,
        ...
        const snapshotFile = session.sessionFile;
        if (!selectedEntry.parentId) {
          sessionManager.newSession(snapshotFile ? { parentSession: snapshotFile } : undefined);
        } else {
      ...
      switchSession: async (sessionPath) => {
        const session = this.requireSession(record);
        const before = await this.emitSessionBeforeSwitch(session, "resume", sessionPath);
        if (before.cancelled) {
          return { cancelled: true };
        }
        ...
        if (sessionCwd && !existsSync(sessionCwd)) {
          throw new Error(`Stored session working directory does not exist: ${sessionCwd}`);
        }
        await this.replaceLiveAgentSession(record, async () => {
          const { session: next } = await this.createAgentSessionImpl({
            cwd: record.workspace.path,
  ...
  private createExtensionUiContext(record: ManagedSessionRecord): ExtensionUIContext {
    ...
    return {
      ...
      custom: async <T>(factory: Parameters<ExtensionUIContext["custom"]>[0]) => {
        const requestId = crypto.randomUUID();
        const columnsForLayout = 88;
        const headlessTerm = createHeadlessTerminal(columnsForLayout, 40);
        ...
        const flushFrame = () => {
          ...
          const lines = component.render(columnsForLayout);
          this.emitHostUiRequest(record, {
            kind: "tuiCustom",
            requestId,
            phase: "frame",
        ...
        return new Promise<T>((resolve, reject) => {
          const done = (result: T) => {
            ...
            record.tuiCustomBridge = undefined;
            try {
          ...
          record.tuiCustomBridge = {
            handleInput: (data: string) => {
              if (settled || !component?.handleInput) {
                return;
              }
          ...
          void Promise.resolve(
            factory(headlessHost, noOpTheme, keybindings as never, done as never),
          )
            .then((c) => {
              component = c;
              flushFrame();
  ...
  private applySessionThinkingLevel(session: AgentSession, thinkingLevel: string): void {
    ...
    const effectiveLevel = clampThinkingLevel(thinkingLevel, availableLevels) as Parameters<AgentSession["setThinkingLevel"]>[0];
  ...
  private emitHostUiRequest(
    ...
    if (isExtensionUiDialogRequest(request)) {
      record.pendingExtensionDialogs.set(request.requestId, request);
    }
  ...
  private clearExtensionUiState(record: ManagedSessionRecord): void {
    ...
    record.extensionUiState.tuiCustom = undefined;
    record.tuiCustomBridge = undefined;
  }
  ...
  private cancelPendingHostUiRequests(record: ManagedSessionRecord): void {
    ...
    record.pendingExtensionDialogs.clear();
    record.tuiCustomBridge = undefined;
  }
...
function formatPiSessionModelLabel(session: AgentSession): string {
  const m = session.model;
  if (!m) return "pi";
  return `${m.provider}/${m.id}`;
}

function inferContextWindowFromModel(model: { id?: string } | undefined, usedTokens: number): number {
  const id = model?.id ?? "";
  if (/1m|1M/i.test(id) || usedTokens > 180_000) {
    return 1_000_000;
  }
  return 200_000;
}

function resolvePiContextWindowSize(usage: ContextUsage | undefined, model: { id?: string } | undefined, usedTokens: number): number {
  if (usage && typeof usage.contextWindow === "number" && usage.contextWindow > 0) {
    return Math.round(usage.contextWindow);
  }
  return inferContextWindowFromModel(model, usedTokens);
}
