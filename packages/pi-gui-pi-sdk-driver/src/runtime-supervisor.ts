...
export class RuntimeSupervisor implements RuntimeResourceDriver {
  ...
  private async autoEnableModelsForAuthenticatedProviders(
    ...
    const providerHasMatchingEnabledModel = (providerId: string): boolean =>
      models.some(
        (model) =>
          model.available &&
    ...
    const authenticatedIds = providers.filter((provider) => provider.hasAuth).map((provider) => provider.id);
    /** Providers with auth but no enabled model row yet (e.g. new Antigravity / custom Cursor after existing patterns). */
    const needsAutoPatterns = authenticatedIds.filter((id) => !providerHasMatchingEnabledModel(id));

    const candidateProviderIds =
      providerIds && providerIds.length > 0 ? [...providerIds] : [...needsAutoPatterns];
