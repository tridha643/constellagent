...
function resolveGitCommonDirSync(repoPath: string): string {
 ...
 */
function parseGraphiteSqliteMetadataSync(gitCommonDir: string): Map<string, string> {
  ...
  try {
    const stdout = execSync(
      `sqlite3 ${JSON.stringify(dbPath)} -separator $'\\t' 'SELECT branch_name, parent_branch_name FROM branch_metadata WHERE parent_branch_name IS NOT NULL AND parent_branch_name != "";'`,
 ...
 */
function parseGraphiteRefMetadataSync(repoPath: string): Map<string, string> {
  const map = new Map<string, string>()
  try {
    const refsOutput = gitSync(
 ...
 */
function parseGraphiteConfigMetadataSync(repoPath: string): Map<string, string> {
  const map = new Map<string, string>()
  try {
 ...
 */
function parseGraphiteMetadataSync(repoPath: string): Map<string, string> {
 ...
 */
export function getGraphiteParentBranch(repoPath: string, branchName: string): string | null {
...
function safeGitOutputSync(args: string[], cwd: string): string {
  try {
...
export function getGraphiteDiffBase(repoPath: string, branchName?: string | null): string {
  if (branchName) {
 ...
 */
export function getAnnotationDiffText(repoPath: string, branchName?: string | null): string {
  ...
  if (diffBase !== 'HEAD') {
    ...
    if (worktreeDiff) parts.push(worktreeDiff)
  }

  return parts.join('\n')
}

export function getDeletedFilesForAnnotationCleanup(
  repoPath: string,
